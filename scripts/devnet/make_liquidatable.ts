...
async function main() {
  ...
  // blockhash miss should not read as a broken deployment.
  const config: DeploymentConfig = await (async () => {
    for (let attempt = 0; attempt < 6; attempt += 1) {
      const response = await fetch(`${API}/api/dusk/v1/config`);
      const body = (await response.json()) as { data?: DeploymentConfig; error?: string };
