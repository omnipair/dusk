...
impl HlpVault {
    ...
    pub fn clear_debt_repay(
        ...
        let debt_reduced_u128 = total_debt
            ...
            .ok_or(ErrorCode::DebtMathOverflow)?;
        require_gte!(debt_reduced_u128, repaid as u128, ErrorCode::DebtMathOverflow);
        ...
        let (principal_paid, interest_paid) = crate::math::realized_interest_split(repaid, total_debt, principal)?;
        let (principal_reduced, _) = crate::math::realized_interest_split(debt_reduced, total_debt, principal)?;
        self.debt_shares = remaining_shares;
        self.debt_principal = self.debt_principal.saturating_sub(principal_reduced as u128);
    ...
    pub fn yield_growth_indexes(&self, market_asset: MarketAsset) -> (u128, u128) {
        match market_asset {
            MarketAsset::Base => (self.base_swap_fee_growth_index_nad, self.base_interest_growth_index_nad),
            MarketAsset::Quote => (
...
impl Market {
    pub fn hlp_yield_growth_indexes(&self, market_asset: MarketAsset) -> (u128, u128) {
        match market_asset {
            ...
            MarketAsset::Quote => self.quote_hlp_vault.yield_growth_indexes(MarketAsset::Quote),
        }
    }
