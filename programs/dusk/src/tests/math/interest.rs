use super::*;
use crate::constants::{
    INTEREST_MAX_ADAPTATION_STEP_NAD, INTEREST_MAX_RATE_AT_TARGET_NAD, INTEREST_MIN_RATE_AT_TARGET_NAD,
};
use crate::state::{
    DEFAULT_IRM_ADJUSTMENT_SPEED_PER_YEAR, DEFAULT_IRM_CURVE_STEEPNESS_NAD,
    DEFAULT_IRM_TARGET_UTILIZATION_BPS,
};

const TARGET: u64 = DEFAULT_IRM_TARGET_UTILIZATION_BPS as u64;
const STEEP: u128 = DEFAULT_IRM_CURVE_STEEPNESS_NAD as u128;
const SPEED: u128 = DEFAULT_IRM_ADJUSTMENT_SPEED_PER_YEAR as u128;
...
fn curve_multiplier_spans_reciprocal_to_steepness() {
    // error 0 -> 1.0
    assert_eq!(instantaneous_rate_apr_nad(NAD as u128, 0, STEEP).unwrap(), NAD as u128);
    // error +1 -> steepness (4x)
    assert_eq!(
        instantaneous_rate_apr_nad(NAD as u128, NAD as i128, STEEP).unwrap(),
        STEEP
    );
...
fn instantaneous_rate_scales_anchor_by_curve() {
    ...
    assert_eq!(instantaneous_rate_apr_nad(anchor, 0, STEEP).unwrap(), anchor);
