use super::*;
use crate::{
    accrue_fee_liability, accrue_fee_liability_with_remainder, constants::YIELD_GROWTH_SCALE_Q64,
};
    ...
    fn fractional_growth_is_backed_once_across_interleaved_transfer_checkpoints() {
        ...
        let (source_first, source_remainder) =
            accrue_fee_liability_with_remainder(1, first_index, 0, 0).unwrap();
        let (destination_first, destination_remainder) =
            accrue_fee_liability_with_remainder(1, first_index, 0, 0).unwrap();
        ...
        let (source_second, source_final_remainder) = accrue_fee_liability_with_remainder(
            0,
            final_index,
            first_index,
            source_remainder,
        ...
        .unwrap();
        let (destination_second, destination_final_remainder) = accrue_fee_liability_with_remainder(
            2,
            final_index,
            first_index,
        ...
        .unwrap();
        let (third_claim, third_remainder) =
            accrue_fee_liability_with_remainder(1, final_index, 0, 0).unwrap();
    ...
    fn distributor_carry_remains_exact_when_supply_changes() {
        ...
        assert_eq!(
            2 * YIELD_GROWTH_SCALE_Q64,
            first_delta * 3 + first_carry as u128
        );
    ...
    fn q64_carry_limits_cohort_drift_below_one_atom_for_large_supply() {
        ...
        for (index, balance) in old_balances.into_iter().enumerate() {
            let (amount, remainder) =
                accrue_fee_liability_with_remainder(balance, old_index, 0, 0).unwrap();
        ...
        for (index, balance) in old_balances.into_iter().enumerate() {
            let (amount, remainder) = accrue_fee_liability_with_remainder(
                balance,
                final_index,
                old_index,
                remainders[index],
        ...
        let represented = (claims as u128)
            ...
            .and_then(|value| {
                value.checked_add(remainders.into_iter().map(u128::from).sum::<u128>())
            })
