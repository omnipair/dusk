...
| --- | ---: | --- |
| hLP passive insolvency and terminal recovery | High | Active-hLP swap-induced combined principal-plus-frozen-public-interest tracking now has an explicit per-operation bound, but hLP funding debt can still grow passively through interest and reach insolvency without a permissionless recovery mechanism or explicit residual-loss waterfall. Reserve conservation, aggregate-funding admission, and swap tracking do not close this terminal risk. The economic design still requires explicit approval before implementation. |
