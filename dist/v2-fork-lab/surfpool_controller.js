...
import { startSurfpoolRelayWorkerProcess, } from "./surfpool_relay_worker.js";
...
const DEFAULT_LEVERAGE_DELEGATE_PROGRAM_ID = "EPGF9iFrbGnhWgC3To9rC9vxinEYuDHaz4RXgLPvuRkp";
...
export function createStartupPhaseTracker() {
    const active = new Set();
    return {
        track(phase) {
            const tracked = Promise.resolve(phase);
            active.add(tracked);
            void tracked.then(() => active.delete(tracked), () => active.delete(tracked));
            return tracked;
        },
...
export class StartupPhaseSettlementError extends Error {
    constructor(timeoutMs, cause) {
        super(`Surfpool startup phases did not settle within ${timeoutMs}ms; ` +
            "refusing in-process Surfnet teardown and requiring a hard process exit", { cause });
        this.name = "StartupPhaseSettlementError";
    }
}
function saturatedAdd(value, increment) {
    return Math.min(Number.MAX_SAFE_INTEGER, value + increment);
}
function positiveSafeInterval(label, value) {
 ...
 */
export function startSurfnetEventDrain(surfnet, options = {}) {
    const minIntervalMs = positiveSafeInterval("Surfnet event drain minimum interval", options.intervalMs ??
        options.minIntervalMs ??
        DEFAULT_EVENT_DRAIN_MIN_INTERVAL_MS);
    const maxIntervalMs = positiveSafeInterval("Surfnet event drain maximum interval", options.intervalMs ??
        options.maxIntervalMs ??
        DEFAULT_EVENT_DRAIN_MAX_INTERVAL_MS);
    if (maxIntervalMs < minIntervalMs) {
        throw new Error("Surfnet event drain maximum interval must be at least its minimum interval");
    }
    const logIntervalMs = positiveSafeInterval("Surfnet event drain log interval", options.logIntervalMs ?? DEFAULT_EVENT_DRAIN_LOG_INTERVAL_MS);
...
function requiredPath(label, candidates) {
...
function requiredPort(label, raw) {
...
function boundedPositiveInteger(label, raw, fallback, maximum) {
...
function requiredRemoteRpcUrl(raw) {
    if (!raw) {
        throw new Error("Hosted Surfpool requires FORK_SDK_REMOTE_RPC_URL or " +
...
export function parsePayerFundingLamports(raw = String(DEFAULT_PAYER_FUNDING_LAMPORTS)) {
    if (!/^\d+$/.test(raw)) {
    ...
    if (!Number.isSafeInteger(lamports) || lamports < 1) {
        throw new Error("SURFPOOL_PAYER_FUNDING_LAMPORTS must be a positive safe integer");
    }
    return lamports;
}
export function decodeExplicitPayer(env = process.env) {
    const raw = env.FORK_LAB_PAYER_KEYPAIR_JSON ?? env.FORK_LAB_PAYER_KEYPAIR_BASE64;
    if (!raw) {
        throw new Error("Hosted Surfpool requires the same explicit FORK_LAB_PAYER_KEYPAIR_JSON " +
...
export function controllerReadinessConfigFromEnv(env = process.env) {
    ...
    if (!host.trim())
        throw new Error("SURFPOOL_HEALTH_HOST must not be empty");
    return {
        host,
        port,
        requestTimeoutMs: boundedPositiveInteger("SURFPOOL_HEALTH_REQUEST_TIMEOUT_MS", env.SURFPOOL_HEALTH_REQUEST_TIMEOUT_MS, DEFAULT_HEALTH_REQUEST_TIMEOUT_MS, 120_000),
        maxConnections: boundedPositiveInteger("SURFPOOL_HEALTH_MAX_CONNECTIONS", env.SURFPOOL_HEALTH_MAX_CONNECTIONS, DEFAULT_HEALTH_MAX_CONNECTIONS, 256),
    };
}
export function hostedControllerConfigFromEnv(env = process.env) {
    const rpcPort = requiredPort("SURFPOOL_RPC_PORT", env.SURFPOOL_RPC_PORT ?? "8899");
    const wsPort = requiredPort("SURFPOOL_WS_PORT", env.SURFPOOL_WS_PORT ?? "8900");
    if (rpcPort === wsPort)
        throw new Error("Surfpool RPC and WebSocket ports must differ");
    return {
        remoteRpcUrl: requiredRemoteRpcUrl(env.FORK_SDK_REMOTE_RPC_URL ?? env.SURFPOOL_DATASOURCE_RPC_URL),
        payer: decodeExplicitPayer(env),
        payerFundingLamports: parsePayerFundingLamports(env.SURFPOOL_PAYER_FUNDING_LAMPORTS),
        listenHost: env.SURFPOOL_HOST ?? "0.0.0.0",
        rpcPort,
        wsPort,
        duskProgramId: env.DUSK_PROGRAM_ID ?? DEFAULT_DUSK_PROGRAM_ID,
        leverageDelegateProgramId: env.DUSK_LEVERAGE_DELEGATE_PROGRAM_ID ??
        ...
        startupProbeTimeoutMs: boundedPositiveInteger("SURFPOOL_STARTUP_PROBE_TIMEOUT_MS", env.SURFPOOL_STARTUP_PROBE_TIMEOUT_MS, DEFAULT_STARTUP_PROBE_TIMEOUT_MS, 120_000),
        startupSettlementTimeoutMs: boundedPositiveInteger("SURFPOOL_STARTUP_SETTLEMENT_TIMEOUT_MS", env.SURFPOOL_STARTUP_SETTLEMENT_TIMEOUT_MS, DEFAULT_STARTUP_SETTLEMENT_TIMEOUT_MS, 300_000),
        relayHeartbeatIntervalMs: boundedPositiveInteger("SURFPOOL_RELAY_HEARTBEAT_INTERVAL_MS", env.SURFPOOL_RELAY_HEARTBEAT_INTERVAL_MS, DEFAULT_RELAY_HEARTBEAT_INTERVAL_MS, 60_000),
    };
}
export function relayWorkerConfigFromEnv(controller, readiness = controllerReadinessConfigFromEnv(), env = process.env) {
    const heartbeatIntervalMs = boundedPositiveInteger("SURFPOOL_RELAY_HEARTBEAT_INTERVAL_MS", env.SURFPOOL_RELAY_HEARTBEAT_INTERVAL_MS, DEFAULT_RELAY_HEARTBEAT_INTERVAL_MS, 60_000);
    const heartbeatStaleMs = boundedPositiveInteger("SURFPOOL_RELAY_HEARTBEAT_STALE_MS", env.SURFPOOL_RELAY_HEARTBEAT_STALE_MS, DEFAULT_RELAY_HEARTBEAT_STALE_MS, 300_000);
    if (heartbeatStaleMs < heartbeatIntervalMs * 2) {
        throw new Error("SURFPOOL_RELAY_HEARTBEAT_STALE_MS must be at least twice " +
    ...
    return {
        ...
        heartbeatStaleMs,
        upstreamConnectTimeoutMs: boundedPositiveInteger("SURFPOOL_RELAY_CONNECT_TIMEOUT_MS", env.SURFPOOL_RELAY_CONNECT_TIMEOUT_MS, DEFAULT_RELAY_CONNECT_TIMEOUT_MS, 120_000),
        maxConnections: boundedPositiveInteger("SURFPOOL_RELAY_MAX_CONNECTIONS", env.SURFPOOL_RELAY_MAX_CONNECTIONS, DEFAULT_RELAY_MAX_CONNECTIONS, 4_096),
        targetProbeIntervalMs: boundedPositiveInteger("SURFPOOL_RELAY_TARGET_PROBE_INTERVAL_MS", env.SURFPOOL_RELAY_TARGET_PROBE_INTERVAL_MS, DEFAULT_RELAY_TARGET_PROBE_INTERVAL_MS, 300_000),
        targetProbeTimeoutMs: boundedPositiveInteger("SURFPOOL_RELAY_TARGET_PROBE_TIMEOUT_MS", env.SURFPOOL_RELAY_TARGET_PROBE_TIMEOUT_MS, DEFAULT_RELAY_TARGET_PROBE_TIMEOUT_MS, 120_000),
        targetProbeFailureThreshold: boundedPositiveInteger("SURFPOOL_RELAY_TARGET_PROBE_FAILURE_THRESHOLD", env.SURFPOOL_RELAY_TARGET_PROBE_FAILURE_THRESHOLD, DEFAULT_RELAY_TARGET_PROBE_FAILURE_THRESHOLD, 20),
    };
}
function sendReadinessJson(response, statusCode, payload, headOnly) {
 ...
 */
export async function startControllerReadinessServer(config = controllerReadinessConfigFromEnv()) {
    ...
    let closing;
    let rejectFailure = () => undefined;
    const failure = new Promise((_resolvePromise, reject) => {
        rejectFailure = reject;
    });
    void failure.catch(() => undefined);
    const snapshot = () => ({
    ...
    const server = http.createServer((request, response) => {
        ...
        if (request.method !== "GET" && request.method !== "HEAD") {
            ...
            sendReadinessJson(response, 405, { ok: false, status: "method_not_allowed" }, headOnly);
            return;
        }
        const current = snapshot();
        sendReadinessJson(response, current.status === "ready" ? 200 : 503, {
...
export function deployExactProgram(surfnet, params) {
    ...
    if (deployedProgramId !== params.programId) {
        throw new Error(`${params.label} deployed at ${deployedProgramId}; expected ${params.programId}`);
    }
    return deployedProgramId;
}
export function rewriteUpgradeableProgramDataAuthority(data, authority) {
    if (data.length <= PROGRAM_DATA_METADATA_BYTES ||
...
export async function alignExactUpgradeableProgramAuthority(surfnet, params) {
    ...
    const [expectedProgramDataAddress] = PublicKey.findProgramAddressSync([programId.toBuffer()], BPF_LOADER_UPGRADEABLE_ID);
    ...
    if (!before.data.subarray(PROGRAM_DATA_METADATA_BYTES).equals(expectedBinary)) {
        throw new Error(`${params.label} ProgramData bytes do not match ${params.soPath}`);
    }
    const rewritten = rewriteUpgradeableProgramDataAuthority(before.data, params.authority);
    const alreadyAligned = before.data.equals(rewritten);
    if (!alreadyAligned) {
        surfnet.setAccount(programDataAddress.toBase58(), before.lamports, rewritten, BPF_LOADER_UPGRADEABLE_ID.toBase58());
    }
    ...
    if (!after ||
        ...
        throw new Error(`${params.label} ProgramData changed outside its upgrade-authority header`);
    }
    if (!after.data.subarray(PROGRAM_DATA_METADATA_BYTES).equals(expectedBinary)) {
        throw new Error(`${params.label} ProgramData binary changed while aligning upgrade authority`);
    }
    return {
...
export async function probeExactUpgradeableProgram(params) {
    ...
    const upgradeAuthority = authorityOption === 1
        ...
        : null;
    if (params.expectedUpgradeAuthority !== undefined &&
        upgradeAuthority !== params.expectedUpgradeAuthority) {
        throw new Error(`${params.label} upgrade authority is ${upgradeAuthority ?? "immutable"}; ` +
...
export async function probeStableWebSocket(wsUrl, timeoutMs = DEFAULT_STARTUP_PROBE_TIMEOUT_MS) {
    const boundedTimeoutMs = boundedPositiveInteger("Surfpool stable WebSocket probe timeout", String(timeoutMs), DEFAULT_STARTUP_PROBE_TIMEOUT_MS, 120_000);
    await new Promise((resolvePromise, reject) => {
        ...
        const timer = setTimeout(() => {
            finish(new Error(`Surfpool stable WebSocket probe timed out after ${boundedTimeoutMs}ms`));
        }, boundedTimeoutMs);
        socket.once("open", () => finish());
        socket.once("error", (error) => finish(new Error(`Surfpool stable WebSocket probe failed: ${error.message}`)));
        socket.once("unexpected-response", (_request, response) => {
            response.resume();
            finish(new Error(`Surfpool stable WebSocket probe received HTTP ${response.statusCode}`));
        });
    });
}
export async function startHostedSurfpoolController(config = hostedControllerConfigFromEnv(), options = {}) {
    const startupPhases = createStartupPhaseTracker();
    let rejectAbort = () => undefined;
    const abortFailure = new Promise((_resolvePromise, reject) => {
        rejectAbort = reject;
    });
    ...
    try {
        ...
        const settled = await startupPhases.settle(config.startupSettlementTimeoutMs ?? DEFAULT_STARTUP_SETTLEMENT_TIMEOUT_MS);
        removeAbortListener();
        if (!settled) {
            throw new StartupPhaseSettlementError(config.startupSettlementTimeoutMs ?? DEFAULT_STARTUP_SETTLEMENT_TIMEOUT_MS, error);
        }
        throw error;
    }
    ...
    let eventDrain;
    let shutdownForkRuntime;
    let relayWorker = options.relayWorker;
    let relayHeartbeatTimer;
    let stopping;
    const stop = () => {
        if (stopping)
            return stopping;
    ...
    try {
        ...
        if (!relayWorker) {
            relayWorker = await startSurfpoolRelayWorkerProcess(relayWorkerConfigFromEnv(config, {
                host: "127.0.0.1",
                port: 0,
        ...
        relayHeartbeatTimer = setInterval(publishRelayHeartbeat, config.relayHeartbeatIntervalMs ?? DEFAULT_RELAY_HEARTBEAT_INTERVAL_MS);
        relayHeartbeatTimer.unref();
        const controllerFailure = Promise.race([
            activeRelayWorker.failure,
            activeEventDrain.failure,
        ]);
        const whileControllerHealthy = (startOperation) => {
            requireStartupActive();
        ...
        requireStartupActive();
        activeEventDrain.drainNow();
        await whileControllerHealthy(() => alignExactUpgradeableProgramAuthority(surfnet, {
        ...
        process.env.FORK_LAB_PAYER_KEYPAIR_JSON = JSON.stringify(Array.from(config.payer.secretKey));
        ...
        const { seedForkGeneration } = (await whileControllerHealthy(() => import("./seed_fork_generation.mjs")));
        ...
        if (markets.length === 0)
            throw new Error("Surfpool controller bootstrapped no markets");
        shutdownForkRuntime();
        activeEventDrain.drainNow();
        ...
        requireStartupActive();
        const duskProgramProbe = startupPhases.track(probeExactUpgradeableProgram({
            rpcUrl: stableRpcUrl,
            label: "dusk",
            programId: config.duskProgramId,
            soPath: config.duskSoPath,
        ...
        const leverageDelegateProgramProbe = startupPhases.track(probeExactUpgradeableProgram({
            rpcUrl: stableRpcUrl,
            label: "leverage_delegate",
            programId: config.leverageDelegateProgramId,
            soPath: config.leverageDelegateSoPath,
        ...
        await whileControllerHealthy(() => probeStableWebSocket(stableWsUrl, config.startupProbeTimeoutMs ?? DEFAULT_STARTUP_PROBE_TIMEOUT_MS));
        ...
        const settlementTimeoutMs = config.startupSettlementTimeoutMs ?? DEFAULT_STARTUP_SETTLEMENT_TIMEOUT_MS;
...
async function main() {
    ...
    const relayWorker = await startSurfpoolRelayWorkerProcess(relayWorkerConfigFromEnv(controllerConfig));
    ...
    try {
        controller = await startHostedSurfpoolController(controllerConfig, { signal: startupAbort.signal, relayWorker });
        if (requestedSignal)
            return;
        console.log(JSON.stringify({
