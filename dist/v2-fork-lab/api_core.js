...
import { ASSOCIATED_TOKEN_PROGRAM_ID, ExtensionType, NATIVE_MINT, TOKEN_2022_PROGRAM_ID, TOKEN_PROGRAM_ID, calculateEpochFee, createAssociatedTokenAccountIdempotentInstruction, createAssociatedTokenAccountInstruction, createInitializeMintInstruction, createInitializeTransferFeeConfigInstruction, createInitializeTransferHookInstruction, createMintToCheckedInstruction, createTransferCheckedInstruction, createTransferCheckedWithTransferHookInstruction, getAccount, getAssociatedTokenAddressSync, getMint, getMintLen, getTransferFeeConfig, } from "@solana/spl-token";
import { ComputeBudgetProgram, Connection, Keypair, LAMPORTS_PER_SOL, PublicKey, SystemProgram, SYSVAR_INSTRUCTIONS_PUBKEY, Transaction, } from "@solana/web3.js";
...
const NAD = 1000000000n;
const DEPLOYMENT_SCHEMA_VERSION = "dusk-deployment.v1";
const DEPLOYMENT_COMMITMENT = "confirmed";
...
function duskEnv(name, fallback) {
...
const HAS_EXPLICIT_PUBLIC_RPC_URL = Boolean(process.env.PUBLIC_SURFPOOL_RPC_URL?.trim() ||
...
export class DeploymentIdentityChangedError extends Error {
    ...
    constructor(message = "Surfpool deployment identity changed during the request") {
        super(message);
        this.name = "DeploymentIdentityChangedError";
    }
}
export class ForkMutationOutcomeUncertainError extends Error {
    code = "fork_mutation_outcome_uncertain";
    httpStatus = 409;
    uncertainOutcome = true;
    constructor(operation, cause) {
        const detail = cause instanceof Error ? cause.message : String(cause);
        super(`${operation} may have been partially applied; reconcile fork state before retrying: ${detail}`);
...
export class ForkAdminAuthConfigurationError extends Error {
    ...
    httpStatus = 503;
    constructor() {
        super("Fork admin API is unavailable because FORK_ADMIN_TOKEN is not configured");
        this.name = "ForkAdminAuthConfigurationError";
    }
}
export class ForkAdminAuthenticationRequiredError extends Error {
    code = "fork_admin_auth_required";
    httpStatus = 401;
    constructor() {
...
export class ForkAdminAuthorizationError extends Error {
    code = "fork_admin_auth_forbidden";
    httpStatus = 403;
    constructor() {
...
function requireActionableLeverageOrderAccount(account) {
...
function requestHeader(req, name) {
    ...
    const value = direct ??
        Object.entries(record).find(([key]) => key.toLowerCase() === normalizedName)?.[1];
    if (Array.isArray(value))
        return value.length === 1 ? value[0] : "";
    return typeof value === "string" ? value : undefined;
}
function isForkAdminPath(rawUrl) {
    const path = new URL(rawUrl ?? "/", "http://localhost").pathname.replace(/\/$/, "") ||
        "/";
    return (path === "/api/v2/fork/admin" || path.startsWith("/api/v2/fork/admin/"));
}
function isForkServerSignedRequest(req, body) {
    const path = new URL(req.url ?? "/", "http://localhost").pathname.replace(/\/$/, "") ||
        "/";
    const bootstrapSigned = body.bootstrapSigned;
    return (path === "/api/v2/fork/tx/bootstrap-rejection" ||
        path === "/api/v2/fork/tx/create-market" ||
        (bootstrapSigned !== undefined &&
            bootstrapSigned !== null &&
            bootstrapSigned !== false));
}
function bootstrapSignedFromBody(value) {
...
export function requireForkAdminAuthorization(req) {
    if (isForkAdminPath(req.url))
        requireForkAdminToken(req);
}
export function requireForkServerSigningAuthorization(req, body) {
...
let leverageDelegateProgram;
let leverageDelegateIdl;
let forkGenerationPromise;
const programBinaryHashPromises = new Map();
let observedRuntimeForkId;
function stateDir() {
...
function protocolTestRunsDir() {
    return resolve(process.env.PROTOCOL_TEST_OUTPUT_DIR ?? ".protocol-test-lab/runs");
}
function protocolTestRunPath(runId) {
    if (!/^[a-zA-Z0-9._-]+$/.test(runId))
        throw new Error("Invalid protocol test run id");
    return resolve(protocolTestRunsDir(), runId, "report.json");
}
function readProtocolTestRun(path) {
    if (!existsSync(path))
        return null;
    return JSON.parse(readFileSync(path, "utf8"));
}
function listProtocolTestRuns() {
...
function normalizeBootstrapTransactions(value) {
    if (!Array.isArray(value))
        return [];
    return mergeBootstrapTransactions(value.flatMap((entry) => {
...
function normalizeForkState(value) {
    if (!value || typeof value !== "object")
        return emptyForkState();
    const record = value;
    const mode = record.futarchyAuthorityBootstrapMode;
    return {
        markets: record.markets && typeof record.markets === "object" && !Array.isArray(record.markets)
            ? record.markets
            : {},
        bootstrapTransactions: normalizeBootstrapTransactions(record.bootstrapTransactions),
        bootstrapEvidenceDeploymentFingerprint: typeof record.bootstrapEvidenceDeploymentFingerprint === "string"
            ? record.bootstrapEvidenceDeploymentFingerprint
            : null,
        futarchyAuthorityBootstrapMode: mode === "transaction" ||
...
function mergeBootstrapTransactions(...collections) {
...
function writeState(state, options = {}) {
    ...
    if (!options.replaceBootstrapEvidence && existsSync(statePath())) {
        ...
        const sameDeployment = !persisted.bootstrapEvidenceDeploymentFingerprint ||
        ...
        if (sameDeployment) {
            next = {
                ...next,
                bootstrapTransactions: mergeBootstrapTransactions(persisted.bootstrapTransactions, next.bootstrapTransactions),
                bootstrapEvidenceDeploymentFingerprint: persisted.bootstrapEvidenceDeploymentFingerprint ??
                    next.bootstrapEvidenceDeploymentFingerprint,
                futarchyAuthorityBootstrapMode: persisted.futarchyAuthorityBootstrapMode ??
...
function beginBootstrapEvidence(deploymentFingerprint) {
...
function recordBootstrapTransaction(label, signature, instructions) {
    const state = readState();
    state.bootstrapTransactions = mergeBootstrapTransactions(state.bootstrapTransactions, [{ label, signature, instructions }]);
    writeState(state, { replaceBootstrapEvidence: true });
}
function recordFutarchyAuthorityBootstrapMode(mode, options = {}) {
    const state = readState();
    if (options.onlyIfMissing && state.futarchyAuthorityBootstrapMode)
        return;
...
function nonBlankEnvironmentValue(value) {
    const trimmed = value?.trim();
    return trimmed ? trimmed : undefined;
}
export function resolvePublicRpcUrl(env = process.env, surfpoolRpcUrl = env.SURFPOOL_RPC_URL ?? "http://127.0.0.1:8899") {
    const explicitPublicRpcUrl = nonBlankEnvironmentValue(env.PUBLIC_SURFPOOL_RPC_URL) ??
    ...
    if (env.DUSK_REQUIRE_PUBLIC_RPC_URL === "true") {
        ...
        if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
            throw new Error("PUBLIC_SURFPOOL_RPC_URL must use http: or https: when " +
...
function publicForkRpcPayload(publicRpcUrl) {
    return { rpcUrl: publicRpcUrl };
}
function createGenerationPinnedGenesisHashReader(load) {
    let pinned;
    return {
        read(generation, confirmGeneration) {
            if (pinned?.generation === generation)
                return pinned.promise;
            const entry = {
                generation,
                promise: Promise.resolve().then(async () => {
                    ...
                    if (confirmedGeneration !== generation) {
                        throw new DeploymentIdentityChangedError("Surfpool fork generation changed while pinning its genesis hash");
                    }
                    return genesisHash;
                }),
            };
...
const lifecycleGenesisHash = createGenerationPinnedGenesisHashReader(() => initializeRuntime().connection.getGenesisHash());
function publicRpcProbeTimeoutMs(raw = process.env.FORK_API_PUBLIC_RPC_PROBE_TIMEOUT_MS) {
    if (raw === undefined || raw === "")
        return DEFAULT_PUBLIC_RPC_PROBE_TIMEOUT_MS;
    const parsed = Number(raw);
    if (!Number.isSafeInteger(parsed) || parsed < 1 || parsed > 60_000) {
        throw new Error("FORK_API_PUBLIC_RPC_PROBE_TIMEOUT_MS must be an integer between 1 and 60000");
    }
    return parsed;
}
async function readBoundedPublicRpcProbeResponse(response) {
...
async function publicRpcProbeRequest(publicRpcUrl, method, params, fetchImpl, timeoutMs) {
...
export async function verifyPublicRpcEndpoint(expected, options = {}) {
    ...
    const filter = await publicRpcProbeRequest(publicRpcUrl, PUBLIC_RPC_FILTER_PROBE_METHOD, [], fetchImpl, timeoutMs);
    const filterError = filter.payload.error;
    if (filter.status !== 403 ||
        !filterError ||
        typeof filterError !== "object" ||
        filterError.code !== -32099) {
        throw new Error("PUBLIC_SURFPOOL_RPC_URL is not the filtered Dusk Surfpool proxy");
    }
    ...
    const health = await publicRpcProbeRequest(publicRpcUrl, "getHealth", [], fetchImpl, timeoutMs);
    ...
    const markerAddress = PublicKey.findProgramAddressSync([Buffer.from("surfpool_fork_generation_v1")], programId)[0];
    const marker = await publicRpcProbeRequest(publicRpcUrl, "getAccountInfo", [
        markerAddress.toBase58(),
        { commitment: DEPLOYMENT_COMMITMENT, encoding: "base64" },
    ], fetchImpl, timeoutMs);
    const markerResult = marker.payload.result;
    const markerValue = markerResult && typeof markerResult === "object"
        ? markerResult.value
        : null;
    ...
    const owner = markerValue.owner;
    const encodedData = markerValue.data;
    if (owner !== programId.toBase58() ||
    ...
    const forkId = deriveForkGenerationId(namespace, expected.genesisHash, programId.toBase58(), markerData);
...
function loadJsonIdl(path, expectedProgramId) {
    const raw = readFileSync(path, "utf8");
    const parsed = JSON.parse(raw);
    const address = String(parsed.address ?? "");
    if (address !== expectedProgramId.toBase58()) {
        throw new Error(`IDL ${path} declares ${address || "no program address"}; expected ${expectedProgramId.toBase58()}`);
    }
    return {
        idl: parsed,
        path,
...
function loadIdl() {
    ...
    throw new Error(`Dusk IDL not found. Tried ${candidates.map((path) => resolve(path)).join(", ")}`);
}
function loadLeverageDelegateIdl() {
    if (leverageDelegateIdl)
    ...
    throw new Error(`Leverage delegate IDL not found. Tried ${candidates.map((path) => resolve(path)).join(", ")}`);
}
function getLeverageDelegateProgram() {
    if (leverageDelegateProgram)
...
function configuredForkControllerKeypair() {
    const inline = process.env.FORK_LAB_PAYER_KEYPAIR_JSON ??
        process.env.FORK_LAB_PAYER_KEYPAIR_BASE64;
    if (inline)
        return parseKeypairSecret(inline);
    const configuredPath = process.env.FORK_LAB_PAYER_KEYPAIR ?? process.env.ANCHOR_WALLET;
    if (!configuredPath)
        return null;
...
function deriveForkKeypair(controller, label) {
    if (!label)
        throw new Error("Derived fork keypair label is required");
    const seed = createHmac("sha256", Buffer.from(controller.secretKey.subarray(0, 32)))
...
function loadOrCreateKeypair(label) {
    ensureStateDir();
    ...
    if (existsSync(path)) {
        ...
        if (deterministic &&
            !Buffer.from(persisted.secretKey).equals(Buffer.from(deterministic.secretKey))) {
            throw new Error(`Persisted hosted fork keypair ${path} does not match deterministic controller derivation`);
        }
        return {
...
function loadPayer() {
    const inline = process.env.FORK_LAB_PAYER_KEYPAIR_JSON ?? process.env.FORK_LAB_PAYER_KEYPAIR_BASE64;
    const materializedPath = resolve(process.env.FORK_LAB_MATERIALIZED_PAYER_PATH ?? `${stateDir()}/payer.json`);
    if (inline) {
    ...
    const configuredKeypairPath = process.env.FORK_LAB_PAYER_KEYPAIR ?? process.env.ANCHOR_WALLET;
    const keypairPath = configuredKeypairPath ?? "deployer-keypair.json";
    const resolved = resolve(keypairPath);
    if (existsSync(resolved))
        return readKeypairFile(resolved);
    if (process.env.DUSK_REQUIRE_EXPLICIT_FORK_SIGNER === "true") {
        throw new Error("Hosted Surfpool requires FORK_LAB_PAYER_KEYPAIR_JSON, " +
...
function tokenMetadataPda(mint) {
    return PublicKey.findProgramAddressSync([seed("metadata"), TOKEN_METADATA_PROGRAM_ID.toBuffer(), mint.toBuffer()], TOKEN_METADATA_PROGRAM_ID)[0];
}
function marketPda(baseMint, quoteMint, paramsHash) {
    return pda(seed("market_v2"), baseMint.toBuffer(), quoteMint.toBuffer(), paramsHash);
}
function deriveMarketAddresses(baseMint, quoteMint, paramsHash) {
...
function deriveLeverageOrder(leveragePosition, positionOwner, orderId) {
    return PublicKey.findProgramAddressSync([seed("leverage_order"), leveragePosition.toBuffer(), positionOwner.toBuffer(), u64Le(orderId)], LEVERAGE_DELEGATE_PROGRAM_ID)[0];
}
function deriveLeverageCustodyAuthority(order) {
    return PublicKey.findProgramAddressSync([seed("leverage_delegate_authority"), order.toBuffer()], LEVERAGE_DELEGATE_PROGRAM_ID)[0];
}
function deriveReferralPartner(authority) {
    return pda(seed("referral_partner"), authority.toBuffer());
}
function deriveReferralAccrual(referralPartner, market, assetMint) {
    return pda(seed("referral_accrual"), referralPartner.toBuffer(), market.toBuffer(), assetMint.toBuffer());
}
function optionalReferralAccounts(value, market, assetMint) {
    if (value == null)
...
async function borrowPositionReferralAccounts(market, positionId, debtAsset, assetMint) {
    const { program } = initializeRuntime();
    const position = await program.account.borrowPosition.fetchNullable(deriveBorrowPosition(market, positionId));
    if (!position)
        return { referralPartner: null, referralAccrual: null };
    return optionalReferralAccounts(debtAsset === "base"
        ? field(position, "baseReferralPartner", "base_referral_partner")
        : field(position, "quoteReferralPartner", "quote_referral_partner"), market, assetMint);
}
async function leveragePositionReferralAccounts(market, positionId, assetMint) {
    const { program } = initializeRuntime();
    const position = await program.account.leveragePosition.fetch(deriveLeveragePosition(market, positionId));
    return optionalReferralAccounts(field(position, "referralPartner", "referral_partner"), market, assetMint);
}
...
function requiredPositionId(body) {
...
function deriveYieldAccount(market, owner, lpMint, assetMint, tokenKind) {
    return pda(seed("yield"), market.toBuffer(), owner.toBuffer(), lpMint.toBuffer(), assetMint.toBuffer(), Buffer.from([tokenKind === "ylp" ? 0 : 1]));
}
function deriveParameterProposal(market, proposer, nonce) {
    return pda(seed("parameter_proposal"), market.toBuffer(), proposer.toBuffer(), u64Le(nonce));
}
function deriveProposalSupport(proposal, supporter) {
    return pda(seed("proposal_support"), proposal.toBuffer(), supporter.toBuffer());
}
function deriveHlpYlpVault(market, targetHlpMint, ylpMint) {
    return pda(seed("hlp_ylp_vault"), market.toBuffer(), targetHlpMint.toBuffer(), ylpMint.toBuffer());
}
function deriveProgramDataAddress() {
    return PublicKey.findProgramAddressSync([PROGRAM_ID.toBuffer()], BPF_LOADER_UPGRADEABLE_ID)[0];
}
function paramsHashForMarket(label, baseMint, quoteMint, kind = "cpmm", allowGenericOverride = true) {
...
function paramsHashForCustomMarket(label, baseMint, quoteMint) {
    return createHash("sha256")
...
async function timeTravel(seconds, slots) {
    ...
    try {
        ...
        if (seconds > 0) {
            ...
            const currentTimestamp = Math.max(blockTime ?? Number.MIN_SAFE_INTEGER, clockUnixTimestamp ?? Number.MIN_SAFE_INTEGER, Math.floor(Date.now() / 1_000));
            absoluteTimestamp =
        ...
        const clockSlotBeforeNormalization = clockBeforeNormalization
            ...
            : null;
        let normalizationSignature = null;
        if (seconds === 0 && slots > 0) {
            const { provider, payer } = initializeRuntime();
            mutationAttempted = true;
            normalizationSignature = await provider.sendAndConfirm(new Transaction().add(ComputeBudgetProgram.setComputeUnitLimit({ units: 200_000 })), [payer]);
        }
...
export async function requestLamportAirdrop(connection, pubkey, lamports) {
    try {
        const signature = await connection.requestAirdrop(pubkey, lamports);
        const confirmation = await connection.confirmTransaction(signature, "confirmed");
        if (confirmation.value.err) {
            throw new Error(`Airdrop transaction ${signature} failed: ${JSON.stringify(confirmation.value.err)}`);
        }
    }
    catch (error) {
...
async function setLamports(pubkey, sol) {
    const lamports = lamportsForSolFunding(sol);
    if (!shouldMutateWalletLamports(sol))
        return;
    const { connection } = initializeRuntime();
    await requestLamportAirdrop(connection, pubkey, lamports);
}
function lamportsForSolFunding(sol) {
    const lamports = sol * LAMPORTS_PER_SOL;
    if (!Number.isSafeInteger(lamports) || lamports < 0) {
        throw new Error("Fork SOL funding must resolve to a nonnegative safe-integer lamport amount");
    }
    return lamports;
}
function shouldMutateWalletLamports(sol) {
    return sol > 0;
}
function requireFundableForkWallet(wallet, existingAccount) {
    if (!PublicKey.isOnCurve(wallet.toBytes())) {
        throw new Error("Fork wallet funding requires an on-curve wallet address");
    }
    if (!existingAccount)
        return;
    if (!existingAccount.owner.equals(SystemProgram.programId) ||
        existingAccount.executable ||
        existingAccount.data.length !== 0) {
        throw new Error("Fork wallet funding requires an absent or plain SystemProgram-owned wallet account");
    }
}
function monotonicForkTokenFundingAmount(current, requested) {
    return requested > current ? requested : current;
}
function additiveForkTokenTopUpAmount(current, requestedMinimum) {
    return requestedMinimum > current ? requestedMinimum - current : 0n;
}
function grossTransferAmountForNet(netAmount, feeForAmount) {
    if (netAmount <= 0n)
        return 0n;
    let low = netAmount;
    let high = BigInt(Number.MAX_SAFE_INTEGER);
    if (high - feeForAmount(high) < netAmount) {
        throw new Error("Fork token top-up cannot satisfy the requested net amount within the JSON safe-integer range");
    }
    while (low < high) {
        const middle = low + (high - low) / 2n;
        if (middle - feeForAmount(middle) >= netAmount)
            high = middle;
        else
            low = middle + 1n;
    }
...
function forkFundingAssetPairMatches(selected, configured) {
    return (selected.baseMint === configured.baseMint &&
...
async function prepareTokenFunding(owner, mint, amount, tokenProgram) {
    ...
    const existing = await connection.getAccountInfo(tokenAccount, DEPLOYMENT_COMMITMENT);
    ...
    if (existing) {
        const decoded = await getAccount(connection, tokenAccount, DEPLOYMENT_COMMITMENT, tokenProgram);
        if (!decoded.isInitialized ||
            decoded.isFrozen ||
            !decoded.mint.equals(mint) ||
            !decoded.owner.equals(owner)) {
            throw new Error("Fork token funding requires an initialized, unfrozen associated token account owned by the requested wallet");
        }
    }
    return {
        owner,
...
async function setTokenBalance(plan) {
    const { owner, mint, requestedMinimum, tokenAccount, tokenProgram, } = plan;
    const { connection, payer, provider } = initializeRuntime();
    const latestTarget = await connection.getAccountInfo(tokenAccount, DEPLOYMENT_COMMITMENT);
    const latestAmount = latestTarget
        ? (await getAccount(connection, tokenAccount, DEPLOYMENT_COMMITMENT, tokenProgram)).amount
        : 0n;
    const netTopUp = additiveForkTokenTopUpAmount(latestAmount, requestedMinimum);
    if (netTopUp === 0n)
        return;
    if (forkMarketFixture() !== "mainnet") {
        ...
        const decimals = (await getMint(connection, mint, DEPLOYMENT_COMMITMENT, tokenProgram)).decimals;
        await provider.sendAndConfirm(new Transaction().add(createMintToCheckedInstruction(mint, tokenAccount, payer.publicKey, netTopUp, decimals, [], tokenProgram)), [payer]);
    }
    else {
        const mintAccount = await getMint(connection, mint, DEPLOYMENT_COMMITMENT, tokenProgram);
        let grossTopUp = netTopUp;
        if (tokenProgram.equals(TOKEN_2022_PROGRAM_ID)) {
            const transferFeeConfig = getTransferFeeConfig(mintAccount);
            if (transferFeeConfig) {
                const epoch = BigInt((await connection.getEpochInfo(DEPLOYMENT_COMMITMENT)).epoch);
                grossTopUp = grossTransferAmountForNet(netTopUp, (candidate) => calculateEpochFee(transferFeeConfig, epoch, candidate));
            }
        }
        ...
        const faucetAuthority = deriveForkKeypair(payer, `public-token-faucet:${mint.toBase58()}:${tokenProgram.toBase58()}`);
        const faucetTokenAccount = getAssociatedTokenAddressSync(mint, faucetAuthority.publicKey, false, tokenProgram);
        ...
        const instructions = [];
        if (!latestTarget) {
            instructions.push(createAssociatedTokenAccountIdempotentInstruction(payer.publicKey, tokenAccount, owner, mint, tokenProgram, ASSOCIATED_TOKEN_PROGRAM_ID));
        }
        instructions.push(tokenProgram.equals(TOKEN_2022_PROGRAM_ID)
            ? await createTransferCheckedWithTransferHookInstruction(connection, faucetTokenAccount, mint, tokenAccount, faucetAuthority.publicKey, grossTopUp, mintAccount.decimals, [], DEPLOYMENT_COMMITMENT, tokenProgram)
            : createTransferCheckedInstruction(faucetTokenAccount, mint, tokenAccount, faucetAuthority.publicKey, grossTopUp, mintAccount.decimals, [], tokenProgram));
        await provider.sendAndConfirm(new Transaction().add(...instructions), [
            payer,
            faucetAuthority,
        ]);
    }
    const finalAmount = (await getAccount(connection, tokenAccount, DEPLOYMENT_COMMITMENT, tokenProgram)).amount;
    if (finalAmount < requestedMinimum) {
        throw new ForkMutationOutcomeUncertainError("Fork token top-up", new Error(`wallet balance ${finalAmount} remains below requested minimum ${requestedMinimum}`));
    }
}
...
function forkGenerationMarkerAddress() {
    return pda(seed("surfpool_fork_generation_v1"));
}
function controllerSignerMarkerAddress() {
    return pda(seed("surfpool_controller_signer_v1"));
}
function seededLiquidityMarkerAddress(market) {
    return pda(seed("surfpool_seeded_liquidity_v1"), market.toBuffer());
}
async function hasSeededLiquidityMarker(market) {
    const { connection } = initializeRuntime();
    const marker = seededLiquidityMarkerAddress(market);
    const account = await connection.getAccountInfo(marker, DEPLOYMENT_COMMITMENT);
    if (!account)
        return false;
    if (!account.owner.equals(PROGRAM_ID)) {
        throw new Error(`Seeded-liquidity marker ${marker.toBase58()} has wrong owner ${account.owner.toBase58()}`);
    }
    if (!account.data.equals(market.toBuffer())) {
        throw new Error(`Seeded-liquidity marker ${marker.toBase58()} has invalid market data`);
    }
    return true;
}
async function recordSeededLiquidity(market) {
    await setRawAccount({
    ...
    if (!(await hasSeededLiquidityMarker(market))) {
        throw new Error(`Seeded-liquidity marker was not persisted for ${market.toBase58()}`);
    }
}
async function verifyControllerSignerMarker() {
    const { connection, payer } = initializeRuntime();
    const marker = controllerSignerMarkerAddress();
    const account = await connection.getAccountInfo(marker, DEPLOYMENT_COMMITMENT);
    if (!account) {
        throw new Error(`Surfpool controller signer marker ${marker.toBase58()} is missing`);
    }
    if (!account.owner.equals(PROGRAM_ID)) {
        throw new Error(`Surfpool controller signer marker has wrong owner ${account.owner.toBase58()}`);
    }
    if (!account.data.equals(payer.publicKey.toBuffer())) {
        throw new Error(`API signer ${payer.publicKey.toBase58()} does not match the RPC bootstrap controller`);
    }
}
async function recordControllerSignerMarker() {
    const { connection, payer } = initializeRuntime();
    const marker = controllerSignerMarkerAddress();
    const existing = await connection.getAccountInfo(marker, DEPLOYMENT_COMMITMENT);
    if (!existing) {
        await setRawAccount({
            pubkey: marker,
            owner: PROGRAM_ID,
...
function deriveForkGenerationId(namespace, genesisHash, programId, markerData) {
    return `surfpool-${sha256(`${namespace}:${genesisHash}:${programId}:${markerData.toString("hex")}`)}`;
}
async function createForkGenerationMarker() {
...
async function observeForkGeneration(minimumContextSlot) {
    const { connection } = initializeRuntime();
    const marker = forkGenerationMarkerAddress();
    ...
    if (!observation.value) {
        if (process.env.DUSK_REQUIRE_EXTERNAL_FORK_MARKER === "true") {
            throw new Error(`Surfpool fork generation marker ${marker.toBase58()} is missing; the RPC service must initialize it`);
        }
    ...
    if (!account.owner.equals(PROGRAM_ID)) {
        throw new Error(`Surfpool fork generation marker has wrong owner ${account.owner.toBase58()}`);
    }
    if (account.data.length !== 32) {
        throw new Error(`Surfpool fork generation marker has invalid length ${account.data.length}; expected 32`);
    }
    return {
...
function deploymentBuildRevision() {
    return (process.env.DUSK_BUILD_REVISION ??
        process.env.RAILWAY_GIT_COMMIT_SHA ??
        process.env.VERCEL_GIT_COMMIT_SHA ??
        "snapshot0-local-unversioned");
}
function resetWeb3TransportCachesAfterForkChange(connection) {
    const mutable = connection;
...
function parseUpgradeableProgramDataHeader(data) {
    if (data.length < 13 ||
    ...
    if (authorityOption === 0)
        return { programDataSlot, upgradeAuthority: null };
...
function cacheProgramBinaryHash(cacheKey, load) {
    ...
    while (programBinaryHashPromises.size > 32) {
        const oldest = programBinaryHashPromises.keys().next().value;
...
async function observeUpgradeableProgram(programId, minimumContextSlot, forkId) {
    const { connection } = initializeRuntime();
    const programObservation = await connection.getAccountInfoAndContext(programId, {
        commitment: DEPLOYMENT_COMMITMENT,
        minContextSlot: minimumContextSlot,
    ...
    if (!programAccount?.executable) {
        throw new Error(`Program ${programId.toBase58()} is missing or not executable`);
    }
    if (!programAccount.owner.equals(BPF_LOADER_UPGRADEABLE_ID)) {
        throw new Error(`Program ${programId.toBase58()} has unsupported loader ${programAccount.owner.toBase58()}`);
    }
    if (programAccount.data.length < 36 ||
        programAccount.data.readUInt32LE(0) !== UPGRADEABLE_PROGRAM_TAG) {
        throw new Error(`Program ${programId.toBase58()} has malformed upgradeable-loader state`);
    }
    const programDataAddress = new PublicKey(programAccount.data.subarray(4, 36));
    const programDataObservation = await connection.getAccountInfoAndContext(programDataAddress, {
        commitment: DEPLOYMENT_COMMITMENT,
        minContextSlot: Math.max(minimumContextSlot, programObservation.context.slot),
    ...
    if (!programDataAccount ||
        ...
        throw new Error(`Program data ${programDataAddress.toBase58()} for ${programId.toBase58()} is missing or has the wrong loader`);
    }
    ...
    const binarySha256 = await cacheProgramBinaryHash(cacheKey, async () => {
        const fullObservation = await connection.getAccountInfoAndContext(programDataAddress, {
            commitment: DEPLOYMENT_COMMITMENT,
        ...
        if (!fullAccount || !fullAccount.owner.equals(BPF_LOADER_UPGRADEABLE_ID)) {
            throw new Error(`Program data ${programDataAddress.toBase58()} changed while hashing ${programId.toBase58()}`);
        }
        if (fullAccount.data.length <= UPGRADEABLE_PROGRAM_DATA_METADATA_BYTES) {
            throw new Error(`Program data ${programDataAddress.toBase58()} for ${programId.toBase58()} has no binary payload`);
        }
        const fullHeader = parseUpgradeableProgramDataHeader(fullAccount.data);
        if (fullHeader.programDataSlot !== header.programDataSlot ||
            fullHeader.upgradeAuthority !== header.upgradeAuthority) {
            throw new DeploymentIdentityChangedError(`Program data ${programDataAddress.toBase58()} changed while its binary was being hashed`);
        }
        return sha256(fullAccount.data.subarray(UPGRADEABLE_PROGRAM_DATA_METADATA_BYTES));
    });
    return {
        programDataAddress: programDataAddress.toBase58(),
        programDataSlot: header.programDataSlot,
        upgradeAuthority: header.upgradeAuthority,
        binarySha256,
        sourceSlot: Math.min(programObservation.context.slot, programDataObservation.context.slot),
    };
}
export async function deploymentEnvelope(minimumSourceSlot = 0) {
    if (!Number.isSafeInteger(minimumSourceSlot) || minimumSourceSlot < 0) {
        throw new Error("Deployment envelope minimum source slot must be a nonnegative safe integer");
    }
    ...
    const genesisHash = await lifecycleGenesisHash.read(forkGeneration.markerHex, async () => (await observeForkGeneration(forkGeneration.sourceSlot)).markerHex);
    const namespace = process.env.DUSK_FORK_NAMESPACE ?? "dusk-surfpool";
    const forkId = deriveForkGenerationId(namespace, genesisHash, PROGRAM_ID.toBase58(), forkGeneration.markerData);
    if (observedRuntimeForkId &&
    ...
    const [duskProgram, delegateProgram] = await Promise.all([
        observeUpgradeableProgram(PROGRAM_ID, minimumSourceSlot, forkId),
        observeUpgradeableProgram(LEVERAGE_DELEGATE_PROGRAM_ID, minimumSourceSlot, forkId),
    ]);
    const sourceSlot = Math.min(duskProgram.sourceSlot, delegateProgram.sourceSlot, forkGeneration.sourceSlot);
    const envelope = {
...
function maximumResponseSourceSlot(value) {
    ...
    const markets = Array.isArray(dataRecord.markets)
        ...
        : typeof dataRecord.marketAddress === "string"
            ? [dataRecord]
            : [];
    return markets.reduce((maximum, market) => {
        ...
        ].reduce((marketMaximum, sourceSlot) => Number.isSafeInteger(sourceSlot) && sourceSlot >= 0
            ? Math.max(marketMaximum, sourceSlot)
            : marketMaximum, maximum);
    }, 0);
}
function deploymentIdentityFingerprint(deployment) {
    return sha256(canonicalJson({
        ...
        leverageDelegateProgramDataAddress: deployment.leverageDelegateProgramDataAddress,
...
async function tokenProgramForMint(mint) {
    ...
    if (!account)
        throw new Error(`Mint account not found in fork: ${mint.toBase58()}`);
    return account.owner.equals(TOKEN_2022_PROGRAM_ID) ? TOKEN_2022_PROGRAM_ID : TOKEN_PROGRAM_ID;
}
async function mintDecimals(mint, tokenProgram) {
    const { connection } = initializeRuntime();
...
async function tokenAccountAmount(tokenAccount, tokenProgram) {
    const { connection } = initializeRuntime();
    try {
...
async function ataInstructionIfMissing(params) {
    const { connection } = initializeRuntime();
    const address = getAssociatedTokenAddressSync(params.mint, params.owner, params.allowOwnerOffCurve ?? false, params.tokenProgram, ASSOCIATED_TOKEN_PROGRAM_ID);
    const existing = await connection.getAccountInfo(address, "confirmed");
    if (existing)
        return { address };
    return {
        address,
        instruction: createAssociatedTokenAccountInstruction(params.payer, address, params.owner, params.mint, params.tokenProgram, ASSOCIATED_TOKEN_PROGRAM_ID),
    };
}
...
function defaultMarketConfig() {
    return {
        ...
        divergenceFeeShareCapBps: Number(duskEnv("DIVERGENCE_FEE_SHARE_CAP_BPS") ?? "0"),
        volatilityFeeShareCapBps: Number(duskEnv("VOLATILITY_FEE_SHARE_CAP_BPS") ?? "0"),
        ...
        directionalEmaHalfLifeMs: toBN(duskEnv("DIRECTIONAL_EMA_HALF_LIFE_MS") ?? "60000"),
        qEmaHalfLifeMs: toBN(duskEnv("Q_EMA_HALF_LIFE_MS") ?? duskEnv("K_EMA_HALF_LIFE_MS") ?? "60000"),
        maxDailyBorrowBps: Number(duskEnv("MAX_DAILY_BORROW_BPS") ?? "2000"),
        globalHealthContributionCapBps: Number(duskEnv("GLOBAL_HEALTH_CONTRIBUTION_CAP_BPS") ?? "15000"),
        borrowMarketHealthFloorBps: Number(duskEnv("BORROW_MARKET_HEALTH_FLOOR_BPS") ?? "11000"),
        amm: defaultAmmConfig(),
        irm: {
            targetUtilizationBps: Number(duskEnv("IRM_TARGET_UTILIZATION_BPS") ?? "7000"),
            curveSteepnessNad: toBN(duskEnv("IRM_CURVE_STEEPNESS_NAD") ?? "4000000000"),
            adjustmentSpeedPerYear: toBN(duskEnv("IRM_ADJUSTMENT_SPEED_PER_YEAR") ?? "20"),
        },
...
function defaultAmmConfig() {
    return {
        ...
        concentratedLiquidityShareNad: toBN(duskEnv("AMM_CONCENTRATED_LIQUIDITY_SHARE_NAD") ?? "0"),
        ...
        divergenceFeeCoefficientNad: toBN(duskEnv("AMM_DIVERGENCE_FEE_COEFFICIENT_NAD") ?? "0"),
        volatilityFeeCoefficientNad: toBN(duskEnv("AMM_VOLATILITY_FEE_COEFFICIENT_NAD") ?? "0"),
        reserved: Array(33).fill(0),
    };
}
function configuredMarketKinds(fixture) {
    const configured = process.env.FORK_BOOTSTRAP_MARKETS ?? (fixture === "mainnet" ? "both" : "cpmm");
    if (configured === "both")
        return ["cpmm", "concentrated"];
    if (configured === "cpmm" || configured === "concentrated")
        return [configured];
    throw new Error(`Unsupported FORK_BOOTSTRAP_MARKETS: ${configured}; expected cpmm, concentrated, or both`);
}
function marketConfigForKind(kind) {
    ...
    return {
        ...
        amm: {
            ...
            concentratedLiquidityShareNad: toBN(duskEnv("AMM_CONCENTRATED_LIQUIDITY_SHARE_NAD") ?? "500000000"),
        },
    };
}
function marketKindFromConfig(config) {
    const amm = field(config, "amm") ?? config?.amm ?? {};
    return toBigInt(field(amm, "rangeWidthNad", "range_width_nad")) > 0n &&
        toBigInt(field(amm, "concentratedLiquidityShareNad", "concentrated_liquidity_share_nad")) > 0n
        ? "concentrated"
        : "cpmm";
}
function bootstrapMarketDefinitions(fixture, baseMint, quoteMint) {
    if (baseMint.equals(quoteMint))
        throw new Error("Dusk fork base and quote mints must differ");
    const kinds = configuredMarketKinds(fixture);
    if (kinds.length > 1 &&
        (duskEnv("FORK_PARAMS_HASH") || duskEnv("MARKET_PARAMS_HASH"))) {
        throw new Error("A shared DUSK_FORK_PARAMS_HASH cannot identify two markets; use " +
...
function defaultLpMetadata(kind) {
    const suffix = kind === "ylp"
        ? "YLP"
        : kind === "baseHlp"
            ? "BASE_HLP"
            : "QUOTE_HLP";
...
async function ensureFutarchyAuthority(futarchyAuthority, onMutationAttempted) {
    ...
    try {
        ...
        const landed = await program.account.futarchyAuthority.fetchNullable(futarchyAuthority);
        if (landed)
            return landed;
        if (process.env.DUSK_ALLOW_SURFPOOL_AUTHORITY_ACCOUNT_SEED !== "true") {
            throw error;
        }
        console.warn(`initFutarchyAuthority failed; explicit Surfpool account-seed fallback is enabled: ${error instanceof Error ? error.message : String(error)}`);
    }
...
async function createHookedLpMintIfMissing(params) {
    ...
    if (!existing) {
        ...
        }), createInitializeTransferHookInstruction(keypair.publicKey, PublicKey.default, PROGRAM_ID, TOKEN_2022_PROGRAM_ID), createInitializeMintInstruction(keypair.publicKey, params.decimals, params.mintAuthority, null, TOKEN_2022_PROGRAM_ID));
...
async function createFixtureAssetMintIfMissing(params) {
    ...
    if (hasTransferFee) {
        transaction.add(createInitializeTransferFeeConfigInstruction(keypair.publicKey, payer.publicKey, payer.publicKey, params.transferFeeBps ?? 0, 1000000000n, TOKEN_2022_PROGRAM_ID));
    }
    transaction.add(createInitializeMintInstruction(keypair.publicKey, params.decimals, payer.publicKey, null, params.tokenProgram));
    transaction.feePayer = payer.publicKey;
    await anchor.web3.sendAndConfirmTransaction(connection, transaction, [payer, keypair], {
        commitment: "confirmed",
    });
...
async function fixtureAssetMints(fixture) {
    if (fixture === "mainnet") {
        return [
...
function deriveTransferHookValidationAddress(lpMint) {
...
async function ensureLpMetadata(params) {
    ...
    if (await connection.getAccountInfo(params.lpTokenMetadata, "confirmed"))
        return;
    const signature = await program.methods
...
async function buildInitializeMarketTx(params) {
    ...
    const teamTreasury = field(field(futarchy, "recipients"), "teamTreasury", "team_treasury") ?? payer.publicKey;
    const teamTreasuryWsolAccount = getAssociatedTokenAddressSync(NATIVE_MINT, teamTreasury, true, TOKEN_PROGRAM_ID);
    const instruction = await program.methods
        .initializeMarket({
...
async function prepareCreateMarketTx(params) {
    ...
    if (params.baseMint.equals(params.quoteMint))
        throw new Error("Choose two different token mints");
    ...
    const paramsHash = paramsHashForCustomMarket(marketLabel, baseMint, quoteMint);
    ...
    try {
        ...
        const futarchy = await ensureFutarchyAuthority(addresses.futarchyAuthority, markMutationAttempted);
        const teamTreasury = field(field(futarchy, "recipients"), "teamTreasury", "team_treasury") ?? payer.publicKey;
        ...
        if (mutationAttempted &&
            ...
            throw new ForkMutationOutcomeUncertainError("Fork market preparation", error);
        }
        throw error;
    }
}
async function buildFinalizeMarketTx(owner, stored) {
    const { program } = initializeRuntime();
    const instructions = await Promise.all([stored.ylpMint, stored.baseHlpMint, stored.quoteHlpMint].map(async (mint) => {
...
async function buildDuplicateMarketTx(stored) {
    const paramsHash = Buffer.from(stored.paramsHash, "hex");
    const addresses = deriveMarketAddresses(new PublicKey(stored.baseMint), new PublicKey(stored.quoteMint), paramsHash);
...
async function buildInvalidConfigMarketTx(stored) {
...
async function bootstrap() {
    ...
    if (!primary)
        throw new Error("Dusk fork bootstrap produced no markets");
    return primary;
}
/** Single-controller entrypoint used by the Surfpool RPC service before API replicas start. */
export async function bootstrapForkMarkets() {
    await recordControllerSignerMarker();
    return bootstrapMarkets();
}
async function bootstrapMarkets(expectedDeploymentFingerprint) {
    initializeRuntime();
    ...
    if (expectedDeploymentFingerprint &&
        ...
        throw new DeploymentIdentityChangedError("Deployment identity changed before market bootstrap");
    }
    ...
    const pending = bootstrapQueue.then(async () => {
        ...
        if (deploymentIdentityFingerprint(before) !== deploymentFingerprint) {
            throw new DeploymentIdentityChangedError("Deployment identity changed while bootstrap was waiting to start");
        }
        const markets = process.env.DUSK_REQUIRE_PREBOOTSTRAPPED_MARKETS === "true"
        ...
        if (deploymentIdentityFingerprint(after) !== deploymentFingerprint) {
            throw new DeploymentIdentityChangedError("Deployment identity changed while bootstrapping markets");
        }
        return markets;
    });
    bootstrapQueue = pending.then(() => undefined, () => undefined);
...
async function bootstrapUncached(deploymentFingerprint) {
...
function expectedLpBootstrapTargets(market) {
    return [
        {
            kind: "ylp",
            mint: market.ylpMint,
...
function publicKeyAt(accountKeys, index) {
    return accountKeys[index]?.toBase58() ?? null;
}
function matchesBootstrapAccounts(accountKeys, expected) {
    return expected.every((value, index) => publicKeyAt(accountKeys, index) ===
        (value instanceof PublicKey ? value.toBase58() : value));
}
function expectedBootstrapInstructionLabel(instructionName, accountKeys, markets, payer) {
    ...
    if (instructionName === "initialize_market") {
        const market = markets.find((candidate) => matchesBootstrapAccounts(accountKeys, [
            payer,
            candidate.baseMint,
            candidate.quoteMint,
            candidate.ylpMint,
...
function bootstrapInstructionName(data) {
    if (data.length < 8)
        return null;
    const instructions = initializeRuntime().idl.instructions ?? [];
...
function transactionAccountKeys(transaction) {
    ...
    ].map((key) => key instanceof PublicKey ? key : key.pubkey);
}
function compiledInstructionData(instruction) {
    return typeof instruction.data === "string"
        ? Buffer.from(anchor.utils.bytes.bs58.decode(instruction.data))
        : Buffer.from(instruction.data ?? []);
}
async function verifiedBootstrapTransaction(signature, markets) {
    const { connection, payer } = initializeRuntime();
    ...
    for (const instruction of compiledInstructions) {
        if (publicKeyAt(transactionKeys, instruction.programIdIndex) !==
            PROGRAM_ID.toBase58()) {
            continue;
        }
        const instructionName = bootstrapInstructionName(compiledInstructionData(instruction));
        if (!instructionName)
            continue;
        const accountIndexes = instruction.accountKeyIndexes ?? instruction.accounts ?? [];
        const instructionAccountKeys = accountIndexes.map((index) => transactionKeys[index]);
        if (instructionAccountKeys.some((key) => !key))
            continue;
        const label = expectedBootstrapInstructionLabel(instructionName, instructionAccountKeys, markets, payer.publicKey);
        if (!label)
            continue;
...
function bootstrapDiscoveryTargets(markets) {
    return [
        {
...
function evidenceMatchesTarget(evidence, target) {
    return evidence.label === target.label &&
        evidence.instructions.includes(target.instruction);
}
async function bootstrapEvidencePayloadUncached(markets, deploymentFingerprint) {
    const { connection } = initializeRuntime();
    const state = readState();
    const stateMatchesDeployment = state.bootstrapEvidenceDeploymentFingerprint === deploymentFingerprint;
    ...
    const verify = (signature) => {
        ...
        if (verificationPromises.size >= maxTransactionFetches) {
            return Promise.reject(new Error(`Bootstrap evidence discovery exceeded ${maxTransactionFetches} unique transaction fetches`));
        }
        const pending = verifiedBootstrapTransaction(signature, markets).then((entry) => {
            if (entry)
                verifiedBySignature.set(signature, entry);
            return entry;
        });
    ...
    for (let offset = 0; offset < persistedTransactions.length; offset += 8) {
        await Promise.all(persistedTransactions.slice(offset, offset + 8).map((transaction) => verify(transaction.signature)));
    }
    const targets = bootstrapDiscoveryTargets(markets);
    const pageSize = 32;
    const maxPagesPerTarget = 8;
    for (const target of targets) {
        if (Array.from(verifiedBySignature.values()).some((entry) => evidenceMatchesTarget(entry.evidence, target))) {
            continue;
        }
        let before;
        let found = false;
        for (let page = 0; page < maxPagesPerTarget && !found; page += 1) {
            let signatures;
            try {
                signatures = await connection.getSignaturesForAddress(target.address, { limit: pageSize, ...(before ? { before } : {}) }, "confirmed");
            }
            catch (error) {
                throw new Error(`Unable to discover ${target.label} from ${target.address.toBase58()}: ${error instanceof Error ? error.message : String(error)}`);
            }
            ...
            for (let offset = 0; offset < successfulSignatures.length; offset += 8) {
                await Promise.all(successfulSignatures.slice(offset, offset + 8).map((signature) => verify(signature.signature)));
                found = Array.from(verifiedBySignature.values()).some((entry) => evidenceMatchesTarget(entry.evidence, target));
                if (found)
                    break;
            }
    ...
    const transactions = mergeBootstrapTransactions(verified.map((entry) => entry.evidence));
    const hasAuthorityTransaction = transactions.some((transaction) => transaction.instructions.includes("init_futarchy_authority"));
    const futarchyAuthorityBootstrapMode = hasAuthorityTransaction
        ...
        : stateMatchesDeployment && state.futarchyAuthorityBootstrapMode
            ...
            : "preexisting";
    const missingTargets = targets.filter((target) => !transactions.some((transaction) => evidenceMatchesTarget(transaction, target))).filter((target) => target.instruction !== "init_futarchy_authority" ||
        futarchyAuthorityBootstrapMode !== "surfpool-account-seed");
    if (missingTargets.length > 0) {
        throw new Error(`Incomplete confirmed bootstrap evidence after bounded fork-history scan: ${missingTargets.map((target) => target.label).join(", ")}`);
    }
    return { transactions, futarchyAuthorityBootstrapMode };
}
async function bootstrapEvidencePayload(markets, deploymentFingerprint) {
    ...
    if (existing)
        return existing;
    const pending = bootstrapEvidencePayloadUncached(markets, deploymentFingerprint);
    const guarded = pending.catch((error) => {
...
function requireMatchingPublicKey(actual, expected, label) {
    if (!actual?.equals(expected)) {
        throw new Error(`${label} mismatch: expected ${expected.toBase58()}, received ${actual?.toBase58() ?? "missing"}`);
    }
}
function prebootstrappedMarketDefinitions() {
    const fixture = forkMarketFixture();
    if (fixture === "mainnet") {
        return bootstrapMarketDefinitions(fixture, new PublicKey(duskEnv("BASE_MINT") ?? DEFAULT_META_MINT), new PublicKey(duskEnv("QUOTE_MINT") ?? DEFAULT_USDC_MINT));
    }
    ...
    if (!saved) {
        throw new Error(`Read-only ${fixture} verification requires the RPC controller state manifest`);
    }
    return bootstrapMarketDefinitions(fixture, new PublicKey(saved.baseMint), new PublicKey(saved.quoteMint));
}
async function requireConfiguredForkFundingAssets(selected) {
    ...
    if (!forkFundingAssetPairMatches(selected, configured)) {
        throw new Error("Public fork funding is restricted to the configured fixture asset pair");
    }
}
async function verifyPrebootstrappedMarket(definition) {
    const { connection, program } = initializeRuntime();
    const addresses = deriveMarketAddresses(definition.baseMint, definition.quoteMint, definition.paramsHash);
    const account = await program.account.market.fetchNullable(addresses.market);
    if (!account) {
        throw new Error(`Market ${addresses.market.toBase58()} is not bootstrapped; the Surfpool RPC controller must initialize it before API replicas start`);
    }
    const baseSide = field(account, "baseSide", "base_side");
    const quoteSide = field(account, "quoteSide", "quote_side");
    const insurance = field(account, "insurance");
    requireMatchingPublicKey(field(baseSide, "assetMint", "asset_mint"), definition.baseMint, `${definition.label} base mint`);
    requireMatchingPublicKey(field(quoteSide, "assetMint", "asset_mint"), definition.quoteMint, `${definition.label} quote mint`);
    const onChainParamsHash = Buffer.from(field(account, "paramsHash", "params_hash") ?? []);
    ...
    const expectedVaults = [
        [
            field(baseSide, "reserveVault", "reserve_vault"),
            addresses.baseReserveVault,
            "base reserve vault",
        ],
        [
            field(quoteSide, "reserveVault", "reserve_vault"),
            addresses.quoteReserveVault,
            "quote reserve vault",
        ],
        [
            field(baseSide, "collateralVault", "collateral_vault"),
            addresses.baseCollateralVault,
            "base collateral vault",
        ],
        [
            field(quoteSide, "collateralVault", "collateral_vault"),
            addresses.quoteCollateralVault,
            "quote collateral vault",
        ],
        [
            field(baseSide, "interestVault", "interest_vault"),
            addresses.baseInterestVault,
            "base interest vault",
        ],
        [
            field(quoteSide, "interestVault", "interest_vault"),
            addresses.quoteInterestVault,
            "quote interest vault",
        ],
        [
            field(insurance, "baseVault", "base_vault"),
            addresses.baseInsuranceVault,
            "base insurance vault",
        ],
        [
            field(insurance, "quoteVault", "quote_vault"),
    ...
    const ylpMint = field(account, "ylpMint", "ylp_mint");
    const baseHlpMint = field(baseSide, "hlpMint", "hlp_mint");
    const quoteHlpMint = field(quoteSide, "hlpMint", "hlp_mint");
    ...
    const requiredObservations = await connection.getMultipleAccountsInfo(requiredAccounts, DEPLOYMENT_COMMITMENT);
    for (let index = 0; index < requiredAccounts.length; index += 1) {
        const observed = requiredObservations[index];
        if (!observed) {
            throw new Error(`${definition.label} prebootstrap account ${requiredAccounts[index].toBase58()} is missing`);
        }
        const expectedOwner = index < 4 ? PROGRAM_ID : TOKEN_METADATA_PROGRAM_ID;
        if (!observed.owner.equals(expectedOwner)) {
            throw new Error(`${definition.label} prebootstrap account ${requiredAccounts[index].toBase58()} has wrong owner`);
        }
    }
    if (!(await hasSeededLiquidityMarker(addresses.market))) {
        throw new Error(`${definition.label} has no seeded-liquidity marker; the RPC controller must finish bootstrap`);
    }
    const baseLiveReserve = toBigInt(field(field(baseSide, "reserves"), "liveReserve", "live_reserve"));
    const quoteLiveReserve = toBigInt(field(field(quoteSide, "reserves"), "liveReserve", "live_reserve"));
...
async function verifyPrebootstrappedMarkets() {
    await verifyControllerSignerMarker();
...
async function bootstrapMarketUncached(definition, state) {
    const { payer, program } = initializeRuntime();
    const { label: marketLabel, kind: marketKind, baseMint, quoteMint, paramsHash, config, } = definition;
    const addresses = deriveMarketAddresses(baseMint, quoteMint, paramsHash);
    ...
    const teamTreasury = field(field(futarchy, "recipients"), "teamTreasury", "team_treasury") ??
        payer.publicKey;
    const teamTreasuryWsolAccount = await createAtaIfMissing({
        payer,
        owner: teamTreasury,
    ...
    let ylpMint = field(existingMarketAccount, "ylpMint", "ylp_mint");
    const existingBaseSide = field(existingMarketAccount, "baseSide", "base_side");
    const existingQuoteSide = field(existingMarketAccount, "quoteSide", "quote_side");
    let baseHlpMint = field(existingBaseSide, "hlpMint", "hlp_mint");
    let quoteHlpMint = field(existingQuoteSide, "hlpMint", "hlp_mint");
    if (!existingMarketAccount) {
    ...
    if (!seededLiquidity && existingMarketAccount) {
        const baseLiveReserve = toBigInt(field(field(existingBaseSide, "reserves"), "liveReserve", "live_reserve"));
        const quoteLiveReserve = toBigInt(field(field(existingQuoteSide, "reserves"), "liveReserve", "live_reserve"));
        if (baseLiveReserve > 0n || quoteLiveReserve > 0n) {
            ...
            throw new Error(`Market ${addresses.market.toBase58()} has no seeded-liquidity marker; the Surfpool RPC controller must finish bootstrap`);
        }
    }
    const stored = {
        label: marketLabel,
...
async function seedInitialLiquidity(market) {
    const { provider, payer } = initializeRuntime();
...
function marketConfigPayload(marketAccount) {
    const config = field(marketAccount, "config");
    const amm = field(config, "amm") ?? defaultAmmConfig();
    const irm = field(config, "irm") ?? defaultMarketConfig().irm;
    return {
        ...
        divergenceFeeShareCapBps: Number(field(config, "divergenceFeeShareCapBps", "divergence_fee_share_cap_bps") ?? 0),
        volatilityFeeShareCapBps: Number(field(config, "volatilityFeeShareCapBps", "volatility_fee_share_cap_bps") ?? 0),
        settlementDivergenceBps: Number(field(config, "settlementDivergenceBps", "settlement_divergence_bps") ?? 0),
        emaHalfLifeMs: stringValue(field(config, "emaHalfLifeMs", "ema_half_life_ms")),
        directionalEmaHalfLifeMs: stringValue(field(config, "directionalEmaHalfLifeMs", "directional_ema_half_life_ms")),
        ...
        globalHealthContributionCapBps: Number(field(config, "globalHealthContributionCapBps", "global_health_contribution_cap_bps") ?? 0),
        borrowMarketHealthFloorBps: Number(field(config, "borrowMarketHealthFloorBps", "borrow_market_health_floor_bps") ?? 0),
        amm: {
            rangeWidthNad: stringValue(field(amm, "rangeWidthNad", "range_width_nad")),
            concentratedLiquidityShareNad: stringValue(field(amm, "concentratedLiquidityShareNad", "concentrated_liquidity_share_nad")),
            centerEmaHalfLifeMs: stringValue(field(amm, "centerEmaHalfLifeMs", "center_ema_half_life_ms")),
            volatilityHalfLifeMs: stringValue(field(amm, "volatilityHalfLifeMs", "volatility_half_life_ms")),
            adjustmentThresholdNad: stringValue(field(amm, "adjustmentThresholdNad", "adjustment_threshold_nad")),
            adjustmentStepNad: stringValue(field(amm, "adjustmentStepNad", "adjustment_step_nad")),
            minAdjustmentIntervalSlots: stringValue(field(amm, "minAdjustmentIntervalSlots", "min_adjustment_interval_slots")),
            volatilityShockCapNad: stringValue(field(amm, "volatilityShockCapNad", "volatility_shock_cap_nad")),
            volatilityCapNad: stringValue(field(amm, "volatilityCapNad", "volatility_cap_nad")),
            divergenceFeeCoefficientNad: stringValue(field(amm, "divergenceFeeCoefficientNad", "divergence_fee_coefficient_nad")),
            volatilityFeeCoefficientNad: stringValue(field(amm, "volatilityFeeCoefficientNad", "volatility_fee_coefficient_nad")),
            reserved: Array.from(field(amm, "reserved") ?? []),
        },
        irm: {
            targetUtilizationBps: Number(field(irm, "targetUtilizationBps", "target_utilization_bps") ?? 0),
            curveSteepnessNad: stringValue(field(irm, "curveSteepnessNad", "curve_steepness_nad")),
            adjustmentSpeedPerYear: stringValue(field(irm, "adjustmentSpeedPerYear", "adjustment_speed_per_year")),
        },
        startTime: stringValue(field(config, "startTime", "start_time")),
    };
}
function protocolAuctionPayload(auction) {
    const recipients = field(auction, "recipients");
    const params = field(auction, "params");
    return {
        ...
        params: {
            ...
            maxReferenceAgeSlots: stringValue(field(params, "maxReferenceAgeSlots", "max_reference_age_slots")),
        },
    };
}
async function futarchyPayload() {
    ...
    const recipients = field(account, "recipients");
    const revenueShare = field(account, "revenueShare", "revenue_share");
    const distribution = field(account, "revenueDistribution", "revenue_distribution");
    const auctionSplit = field(account, "protocolAuctionSplit", "protocol_auction_split");
    return {
        ...
        maxReferralInterestShareBps: Number(field(account, "maxReferralInterestShareBps", "max_referral_interest_share_bps") ?? 0),
        revenueShare: {
        ...
        revenueDistribution: {
            futarchyTreasuryBps: Number(field(distribution, "futarchyTreasuryBps", "futarchy_treasury_bps") ?? 0),
...
async function yieldAccountPayload(stored, owner, lpMint, asset, tokenKind) {
    const { program } = initializeRuntime();
    ...
    return {
        ...
        swapFeeCheckpointQ64: stringValue(field(account, "swapFeeCheckpointQ64", "swap_fee_checkpoint_q64")),
        interestCheckpointQ64: stringValue(field(account, "interestCheckpointQ64", "interest_checkpoint_q64")),
        accruedSwapFeeAmount: stringValue(field(account, "accruedSwapFeeAmount", "accrued_swap_fee_amount")),
        accruedInterestAmount: stringValue(field(account, "accruedInterestAmount", "accrued_interest_amount")),
        bump: Number(field(account, "bump")),
    };
}
async function currentMarketHealth(market) {
    ...
    const transaction = new Transaction().add(ComputeBudgetProgram.requestHeapFrame({ bytes: 256 * 1024 }), ComputeBudgetProgram.setComputeUnitLimit({ units: 1_400_000 }), instruction);
    transaction.feePayer = payer.publicKey;
    ...
    const preview = program.coder.types.decode("marketPreview", Buffer.from(returnData.data[0], returnData.data[1]));
    return {
        health: field(preview, "health"),
        sourceSlot: simulation.context.slot,
    };
}
async function marketPayload(stored) {
    const { connection, program } = initializeRuntime();
    const marketAddress = new PublicKey(stored.market);
    const healthObservation = await currentMarketHealth(marketAddress);
    const marketObservation = await program.account.market.fetchAndContext(marketAddress, DEPLOYMENT_COMMITMENT);
    ...
    const healthBlockTime = await connection.getBlockTime(healthObservation.sourceSlot);
    const sourceObservedAt = sourceBlockTime === null
        ? null
        : new Date(sourceBlockTime * 1_000).toISOString();
    const healthObservedAt = healthBlockTime === null
        ? null
        : new Date(healthBlockTime * 1_000).toISOString();
    const config = marketConfigPayload(marketAccount);
    const baseSide = field(marketAccount, "baseSide", "base_side");
    const quoteSide = field(marketAccount, "quoteSide", "quote_side");
    ...
    const baseDailyBorrowBucket = field(baseSide, "dailyBorrowBucket", "daily_borrow_bucket");
    const quoteDailyBorrowBucket = field(quoteSide, "dailyBorrowBucket", "daily_borrow_bucket");
    ...
    return {
        ...
        config,
        governanceLockedYlp: stringValue(field(marketAccount, "governanceLockedYlp", "governance_locked_ylp")),
        parameterRevisions: Array.from(field(marketAccount, "parameterRevisions", "parameter_revisions") ?? []).map(stringValue),
        ...
        state: {
            ...
            baseSwapFeeCustodyBalance: stringValue(field(baseFees, "swapFeeCustodyBalance", "swap_fee_custody_balance")),
            quoteSwapFeeCustodyBalance: stringValue(field(quoteFees, "swapFeeCustodyBalance", "swap_fee_custody_balance")),
            baseSwapProtocolFeeLiability: stringValue(field(baseFees, "swapProtocolFeeLiability", "swap_protocol_fee_liability")),
            quoteSwapProtocolFeeLiability: stringValue(field(quoteFees, "swapProtocolFeeLiability", "swap_protocol_fee_liability")),
            baseInterestProtocolFeeLiability: stringValue(field(baseFees, "interestProtocolFeeLiability", "interest_protocol_fee_liability")),
            quoteInterestProtocolFeeLiability: stringValue(field(quoteFees, "interestProtocolFeeLiability", "interest_protocol_fee_liability")),
            baseSwapBuybackFeeLiability: stringValue(field(baseFees, "swapBuybackFeeLiability", "swap_buyback_fee_liability")),
            quoteSwapBuybackFeeLiability: stringValue(field(quoteFees, "swapBuybackFeeLiability", "swap_buyback_fee_liability")),
            baseInterestBuybackFeeLiability: stringValue(field(baseFees, "interestBuybackFeeLiability", "interest_buyback_fee_liability")),
            quoteInterestBuybackFeeLiability: stringValue(field(quoteFees, "interestBuybackFeeLiability", "interest_buyback_fee_liability")),
            ...
            baseUnallocatedSwapFeeLiability: stringValue(field(baseFees, "unallocatedSwapFeeLiability", "unallocated_swap_fee_liability")),
            quoteUnallocatedSwapFeeLiability: stringValue(field(quoteFees, "unallocatedSwapFeeLiability", "unallocated_swap_fee_liability")),
            baseDailyBorrowedBucket: stringValue(field(baseDailyBorrowBucket, "borrowedBucket", "borrowed_bucket")),
            quoteDailyBorrowedBucket: stringValue(field(quoteDailyBorrowBucket, "borrowedBucket", "borrowed_bucket")),
            baseDailyLastDecaySlot: stringValue(field(baseDailyBorrowBucket, "lastDecaySlot", "last_decay_slot")),
            quoteDailyLastDecaySlot: stringValue(field(quoteDailyBorrowBucket, "lastDecaySlot", "last_decay_slot")),
            baseDailyDecayRemainderMs: stringValue(field(baseDailyBorrowBucket, "decayRemainderMs", "decay_remainder_ms")),
            quoteDailyDecayRemainderMs: stringValue(field(quoteDailyBorrowBucket, "decayRemainderMs", "decay_remainder_ms")),
            globalHealthBaseContributionForQuoteDebt: stringValue(field(debt, "globalHealthBaseContributionForQuoteDebt", "global_health_base_contribution_for_quote_debt")),
            globalHealthQuoteContributionForBaseDebt: stringValue(field(debt, "globalHealthQuoteContributionForBaseDebt", "global_health_quote_contribution_for_base_debt")),
            effectiveBaseDebtNad: stringValue(field(health, "effectiveBaseDebtNad", "effective_base_debt_nad")),
            effectiveQuoteDebtNad: stringValue(field(health, "effectiveQuoteDebtNad", "effective_quote_debt_nad")),
...
function forkConfigPayload(stored, markets) {
    return {
...
function marketFromStored(stored) {
    return {
        market: new PublicKey(stored.market),
        futarchyAuthority: pda(seed("futarchy_authority")),
...
function hlpSwapRemainingAccountPrefix(addresses, marketAccount) {
    ...
    const active = toBigInt(field(baseHlpVault, "hlpSupply", "hlp_supply")) > 0n ||
...
async function hlpSwapRemainingAccounts(addresses) {
    const { program } = initializeRuntime();
...
async function resolveStoredMarket(marketAddress, fallback) {
    if (!marketAddress || marketAddress === fallback.market)
        return fallback;
    const state = readState();
    ...
    const quoteSide = field(account, "quoteSide", "quote_side");
    const insurance = field(account, "insurance");
    const baseMint = field(baseSide, "assetMint", "asset_mint");
    const quoteMint = field(quoteSide, "assetMint", "asset_mint");
    const ylpMint = field(account, "ylpMint", "ylp_mint");
    const baseHlpMint = field(baseSide, "hlpMint", "hlp_mint");
    const quoteHlpMint = field(quoteSide, "hlpMint", "hlp_mint");
    const paramsHashValue = field(account, "paramsHash", "params_hash");
    ...
    const addresses = deriveMarketAddresses(baseMint, quoteMint, paramsHash);
    if (!addresses.market.equals(market))
        throw new Error(`Market PDA does not match ${marketAddress}`);
    const [baseTokenProgram, quoteTokenProgram] = await Promise.all([
        tokenProgramForMint(baseMint),
        tokenProgramForMint(quoteMint),
    ]);
    const stored = storedMarketDefinition({
        ...
        addresses: {
            ...addresses,
            baseReserveVault: field(baseSide, "reserveVault", "reserve_vault") ?? addresses.baseReserveVault,
            quoteReserveVault: field(quoteSide, "reserveVault", "reserve_vault") ?? addresses.quoteReserveVault,
            baseCollateralVault: field(baseSide, "collateralVault", "collateral_vault") ?? addresses.baseCollateralVault,
            quoteCollateralVault: field(quoteSide, "collateralVault", "collateral_vault") ?? addresses.quoteCollateralVault,
            baseInsuranceVault: field(insurance, "baseVault", "base_vault") ?? addresses.baseInsuranceVault,
            quoteInsuranceVault: field(insurance, "quoteVault", "quote_vault") ?? addresses.quoteInsuranceVault,
            baseInterestVault: field(baseSide, "interestVault", "interest_vault") ?? addresses.baseInterestVault,
            quoteInterestVault: field(quoteSide, "interestVault", "interest_vault") ?? addresses.quoteInterestVault,
        },
        paramsHash,
        baseMint,
        quoteMint,
        ...
        baseHlpMint,
        quoteHlpMint,
        seededLiquidity: toBigInt(field(field(baseSide, "reserves"), "liveReserve", "live_reserve")) > 0n ||
...
async function ownerTransaction(owner, instructions, payerCanSign = false) {
    const { connection, payer } = initializeRuntime();
    const tx = new Transaction();
    tx.add(ComputeBudgetProgram.requestHeapFrame({ bytes: 256 * 1024 }), ComputeBudgetProgram.setComputeUnitLimit({ units: 1_400_000 }), ...instructions);
    tx.feePayer = payerCanSign ? payer.publicKey : owner;
    tx.recentBlockhash = (await connection.getLatestBlockhash("confirmed")).blockhash;
    return tx;
}
async function serializeOwnerTransaction(owner, instructions) {
    const tx = await ownerTransaction(owner, instructions);
    return tx.serialize({ requireAllSignatures: false, verifySignatures: false }).toString("base64");
}
async function serializeBootstrapTransaction(instructions) {
    const { payer } = initializeRuntime();
    const tx = await ownerTransaction(payer.publicKey, instructions, true);
    tx.sign(payer);
    return tx.serialize().toString("base64");
}
function rawAmount(body, keys, decimals, fallback) {
...
function assetFromBody(value, fallback) {
    if (value === "base" || value === "quote")
        return value;
    return fallback;
}
function yieldTokenKindFromBody(value, fallback) {
    if (value === "ylp" || value === "hlp")
        return value;
    return fallback;
}
function protocolAuctionLaneFromBody(value, fallback) {
    if (value === "fee" || value === "buyback")
        return value;
    return fallback;
}
function protocolAuctionLaneArg(lane) {
    return lane === "fee" ? { fee: {} } : { buyback: {} };
}
function protocolRevenueSourceFromBody(value) {
    if (value === "swap" || value === "interest")
        return value;
    throw new Error('Protocol auction source must be explicitly set to "swap" or "interest"');
}
function protocolRevenueSourceArg(source) {
    return source === "swap" ? { swap: {} } : { interest: {} };
}
function protocolRevenueVault(market, source, soldIsBase) {
    if (source === "swap") {
...
async function maybeAddAta(instructions, owner, mint, tokenProgram) {
    const ata = await ataInstructionIfMissing({ payer: owner, owner, mint, tokenProgram });
    if (ata.instruction)
        instructions.push(ata.instruction);
    return ata.address;
}
async function buildPreviewMarketTx(owner, market) {
    const { program } = initializeRuntime();
...
async function buildSwapTx(params) {
    ...
    const inIsBase = params.assetIn === "base";
    const instructions = [];
    const ownerIn = await maybeAddAta(instructions, params.owner, inIsBase ? m.baseMint : m.quoteMint, inIsBase ? m.baseTokenProgram : m.quoteTokenProgram);
    const ownerOut = await maybeAddAta(instructions, params.owner, inIsBase ? m.quoteMint : m.baseMint, inIsBase ? m.quoteTokenProgram : m.baseTokenProgram);
...
async function buildDepositCollateralTx(params) {
    ...
    const instructions = [];
    const ownerAsset = await maybeAddAta(instructions, params.owner, isBase ? m.baseMint : m.quoteMint, isBase ? m.baseTokenProgram : m.quoteTokenProgram);
    instructions.push(await program.methods
        .depositCollateral({
...
async function buildBorrowTx(params) {
    ...
    const instructions = [];
    const ownerDebt = await maybeAddAta(instructions, params.owner, isBase ? m.baseMint : m.quoteMint, isBase ? m.baseTokenProgram : m.quoteTokenProgram);
    const debtMint = isBase ? m.baseMint : m.quoteMint;
    const boundReferral = await borrowPositionReferralAccounts(m.market, params.positionId, params.borrowAsset, debtMint);
...
async function buildInitializeReferralAccrualInstruction(params) {
    const { program } = initializeRuntime();
    const referralAccrual = deriveReferralAccrual(params.referralPartner, params.market, params.assetMint);
    return {
        referralAccrual,
...
async function buildClaimReferralInterestTx(params) {
    ...
    const recipient = new PublicKey(stringValue(field(partner, "recipient")));
    const referralAccrual = deriveReferralAccrual(referralPartner, m.market, params.assetMint);
    ...
    if (recipientAccountResult.instruction)
        instructions.push(recipientAccountResult.instruction);
    ...
    if (params.tokenProgram.equals(TOKEN_2022_PROGRAM_ID)) {
        ...
        const hookTransfer = await createTransferCheckedWithTransferHookInstruction(connection, interestVault, params.assetMint, recipientAccount, m.market, toBigInt(field(accrual, "amount")), mint.decimals, [], "confirmed", params.tokenProgram);
...
async function buildClaimYieldTx(params) {
    ...
    const lpMint = params.tokenKind === "ylp"
        ...
        : isBase
            ...
            : m.quoteHlpMint;
    const instructions = [];
    const ownerLpAccount = await maybeAddAta(instructions, params.owner, lpMint, TOKEN_2022_PROGRAM_ID);
    const recipientAssetAccount = await maybeAddAta(instructions, params.recipient, assetMint, assetTokenProgram);
    instructions.push(await program.methods
...
function parameterUpdateFromBody(update) {
    ...
    if (recognizedFamilies.length !== 1) {
        throw new Error("update must contain exactly one of fee, concentration, irm, emaHalfLives, or dailyBorrowLimit");
    }
    ...
    if (update.concentration && typeof update.concentration === "object") {
        const concentration = update.concentration;
        return {
            concentration: {
                rangeWidthNad: toBN(String(concentration.rangeWidthNad)),
                concentratedLiquidityShareNad: toBN(String(concentration.concentratedLiquidityShareNad)),
            },
        };
    }
    if (update.irm && typeof update.irm === "object") {
        const irm = update.irm;
        return {
            irm: {
                0: {
    ...
    if (update.dailyBorrowLimit && typeof update.dailyBorrowLimit === "object") {
        const daily = update.dailyBorrowLimit;
        return {
            dailyBorrowLimit: {
...
function proposalMetadataFromBody(metadata) {
    ...
    return {
        ...
        descriptionUri,
        descriptionSha256,
        descriptionLen: Number(metadata?.descriptionLen ?? Buffer.byteLength(description, "utf8")),
    };
}
async function buildCreateParameterProposalTx(params) {
    ...
    const proposerYlpAccount = getAssociatedTokenAddressSync(m.ylpMint, params.proposer, false, TOKEN_2022_PROGRAM_ID);
    const instruction = await program.methods
        ...
        ylpMint: m.ylpMint,
        proposerYlpAccount,
        baseYieldAccount: deriveYieldAccount(m.market, params.proposer, m.ylpMint, m.baseMint, "ylp"),
        quoteYieldAccount: deriveYieldAccount(m.market, params.proposer, m.ylpMint, m.quoteMint, "ylp"),
...
async function buildSupportParameterProposalTx(params) {
    ...
    const supporterYlpAccount = getAssociatedTokenAddressSync(m.ylpMint, params.supporter, false, TOKEN_2022_PROGRAM_ID);
    const instruction = await program.methods
        ...
        ylpMint: m.ylpMint,
        supporterYlpAccount,
        baseYieldAccount: deriveYieldAccount(m.market, params.supporter, m.ylpMint, m.baseMint, "ylp"),
        quoteYieldAccount: deriveYieldAccount(m.market, params.supporter, m.ylpMint, m.quoteMint, "ylp"),
        baseHlpYlpVault: new PublicKey(params.market.baseHlpYlpVault),
        quoteHlpYlpVault: new PublicKey(params.market.quoteHlpYlpVault),
        token2022Program: TOKEN_2022_PROGRAM_ID,
        systemProgram: SystemProgram.programId,
...
async function buildTransferLpTx(params) {
    ...
    const instructions = [];
    const source = await maybeAddAta(instructions, params.owner, lpMint, TOKEN_2022_PROGRAM_ID);
    ...
    if (destination.instruction)
        instructions.push(destination.instruction);
    instructions.push(await createTransferCheckedWithTransferHookInstruction(connection, source, lpMint, destination.address, params.owner, params.amount, mint.decimals, [], "confirmed", TOKEN_2022_PROGRAM_ID));
    return serializeOwnerTransaction(params.owner, instructions);
}
...
async function buildSettleProtocolAuctionTx(params) {
    ...
    if (bidderPayment.instruction)
        instructions.push(bidderPayment.instruction);
    ...
    if (bidderReceive.instruction)
        instructions.push(bidderReceive.instruction);
    ...
    if (treasuryPayment.instruction)
        instructions.push(treasuryPayment.instruction);
    ...
    if (stakingPayment.instruction)
        instructions.push(stakingPayment.instruction);
    instructions.push(await program.methods
...
function boundedIntegerNumber(value, label, minimum, maximum) {
...
function boundedIntegerBn(value, label, minimum, maximum) {
    const text = integerText(value, label);
    const parsed = BigInt(text);
    if (parsed < minimum || parsed > maximum) {
        throw new Error(`${label} must be between ${minimum.toString()} and ${maximum.toString()}`);
    }
    return new BN(text, 10);
}
function marketConfigFromBody(config) {
    const amm = config.amm ?? defaultAmmConfig();
    const defaultIrm = defaultMarketConfig().irm;
    const irm = config.irm ?? defaultIrm;
    const u16 = (value, label) => boundedIntegerNumber(value, label, 0, 65_535);
    const u64 = (value, label) => boundedIntegerBn(value, label, 0n, (1n << 64n) - 1n);
    const i64 = (value, label) => boundedIntegerBn(value, label, -(1n << 63n), (1n << 63n) - 1n);
    const reserved = Array.isArray(amm.reserved)
        ? amm.reserved.map((value, index) => boundedIntegerNumber(value, `config.amm.reserved[${index}]`, 0, 255))
        : Array(33).fill(0);
    ...
    return {
        ...
        divergenceFeeShareCapBps: u16(config.divergenceFeeShareCapBps ?? 0, "config.divergenceFeeShareCapBps"),
        volatilityFeeShareCapBps: u16(config.volatilityFeeShareCapBps ?? 0, "config.volatilityFeeShareCapBps"),
        targetHlpLeverageBps: u16(config.targetHlpLeverageBps, "config.targetHlpLeverageBps"),
        settlementDivergenceBps: u16(config.settlementDivergenceBps, "config.settlementDivergenceBps"),
        emaHalfLifeMs: u64(config.emaHalfLifeMs, "config.emaHalfLifeMs"),
        directionalEmaHalfLifeMs: u64(config.directionalEmaHalfLifeMs, "config.directionalEmaHalfLifeMs"),
        qEmaHalfLifeMs: u64(config.qEmaHalfLifeMs, "config.qEmaHalfLifeMs"),
        maxDailyBorrowBps: u16(config.maxDailyBorrowBps, "config.maxDailyBorrowBps"),
        globalHealthContributionCapBps: u16(config.globalHealthContributionCapBps, "config.globalHealthContributionCapBps"),
        borrowMarketHealthFloorBps: u16(config.borrowMarketHealthFloorBps, "config.borrowMarketHealthFloorBps"),
        amm: {
            rangeWidthNad: u64(amm.rangeWidthNad, "config.amm.rangeWidthNad"),
            concentratedLiquidityShareNad: u64(amm.concentratedLiquidityShareNad, "config.amm.concentratedLiquidityShareNad"),
            centerEmaHalfLifeMs: u64(amm.centerEmaHalfLifeMs, "config.amm.centerEmaHalfLifeMs"),
            volatilityHalfLifeMs: u64(amm.volatilityHalfLifeMs, "config.amm.volatilityHalfLifeMs"),
            adjustmentThresholdNad: u64(amm.adjustmentThresholdNad, "config.amm.adjustmentThresholdNad"),
            adjustmentStepNad: u64(amm.adjustmentStepNad, "config.amm.adjustmentStepNad"),
            minAdjustmentIntervalSlots: u64(amm.minAdjustmentIntervalSlots, "config.amm.minAdjustmentIntervalSlots"),
            volatilityShockCapNad: u64(amm.volatilityShockCapNad, "config.amm.volatilityShockCapNad"),
            volatilityCapNad: u64(amm.volatilityCapNad, "config.amm.volatilityCapNad"),
            divergenceFeeCoefficientNad: u64(amm.divergenceFeeCoefficientNad, "config.amm.divergenceFeeCoefficientNad"),
            volatilityFeeCoefficientNad: u64(amm.volatilityFeeCoefficientNad, "config.amm.volatilityFeeCoefficientNad"),
            reserved,
        },
        irm: {
            targetUtilizationBps: u16(irm.targetUtilizationBps ?? defaultIrm.targetUtilizationBps, "config.irm.targetUtilizationBps"),
            curveSteepnessNad: u64(irm.curveSteepnessNad ?? defaultIrm.curveSteepnessNad, "config.irm.curveSteepnessNad"),
            adjustmentSpeedPerYear: u64(irm.adjustmentSpeedPerYear ?? defaultIrm.adjustmentSpeedPerYear, "config.irm.adjustmentSpeedPerYear"),
        },
...
async function buildRepayTx(params) {
    ...
    const debtMint = isBase ? m.baseMint : m.quoteMint;
    const referral = await borrowPositionReferralAccounts(m.market, params.positionId, params.repayAsset, debtMint);
    const instructions = [];
    const ownerDebt = await maybeAddAta(instructions, params.owner, debtMint, isBase ? m.baseTokenProgram : m.quoteTokenProgram);
    instructions.push(await program.methods
        .repay({
...
async function buildOpenLeverageTx(params) {
    ...
    if (hLpAccounts.length > 0)
        builder = builder.remainingAccounts(hLpAccounts);
    instructions.push(await builder.instruction());
    return {
...
async function buildDecreaseLeverageTx(params) {
    ...
    const collateralMint = debtIsBase ? m.quoteMint : m.baseMint;
    const leveragePosition = deriveLeveragePosition(m.market, params.positionId);
    const referral = await leveragePositionReferralAccounts(m.market, params.positionId, debtMint);
    const leverageCollateralVault = deriveLeverageCollateralVault(m.market, collateralMint);
    let builder = program.methods
        .decreaseLeverage({
        debtAsset: debtIsBase ? 0 : 1,
...
async function buildDelegatedCloseLeverageTx(params) {
    ...
    requireActionableLeverageOrderAccount(await provider.connection.getAccountInfo(order, DEPLOYMENT_COMMITMENT));
    const referral = await leveragePositionReferralAccounts(m.market, params.positionId, debtMint);
    const leverageDelegation = deriveLeverageDelegation(leveragePosition);
    ...
    if (custodyAccount.instruction)
        instructions.push(custodyAccount.instruction);
    ...
    if (executorAccount.instruction)
        instructions.push(executorAccount.instruction);
    ...
    if (ownerAccount.instruction)
        instructions.push(ownerAccount.instruction);
...
async function buildLiquidateLeverageTx(params) {
    ...
    const debtTokenProgram = debtIsBase ? m.baseTokenProgram : m.quoteTokenProgram;
    const instructions = [];
    const liquidatorDebtAccount = await maybeAddAta(instructions, params.liquidator, debtMint, debtTokenProgram);
    ...
    if (ownerDebtAccountResult.instruction)
        instructions.push(ownerDebtAccountResult.instruction);
    const leveragePosition = deriveLeveragePosition(m.market, params.positionId);
    const referral = await leveragePositionReferralAccounts(m.market, params.positionId, debtMint);
    let builder = program.methods
...
async function buildBidLiquidationAuctionTx(params) {
    ...
    const debtMint = debtIsBase ? m.baseMint : m.quoteMint;
    const collateralMint = debtIsBase ? m.quoteMint : m.baseMint;
    const referral = await borrowPositionReferralAccounts(m.market, params.positionId, params.debtAsset, debtMint);
    const instructions = [];
    const liquidatorDebtAccount = await maybeAddAta(instructions, params.liquidator, debtMint, debtIsBase ? m.baseTokenProgram : m.quoteTokenProgram);
    const liquidatorCollateralAccount = await maybeAddAta(instructions, params.liquidator, collateralMint, debtIsBase ? m.quoteTokenProgram : m.baseTokenProgram);
    instructions.push(await program.methods
...
async function buildDepositSingleSidedTx(params) {
    ...
    const instructions = [];
    const ownerTarget = await maybeAddAta(instructions, params.owner, isBase ? m.baseMint : m.quoteMint, isBase ? m.baseTokenProgram : m.quoteTokenProgram);
    const ownerHlp = await maybeAddAta(instructions, params.owner, isBase ? m.baseHlpMint : m.quoteHlpMint, TOKEN_2022_PROGRAM_ID);
...
async function userPositionsPayload(wallet, stored, positionId = null) {
    const { program } = initializeRuntime();
    ...
    if (!positionId)
        return positions;
    const projectionId = (eventType, address) => createHash("sha256")
...
let walletFundingQueue = Promise.resolve();
function serializeWalletFunding(operation) {
    const pending = walletFundingQueue.then(operation);
    walletFundingQueue = pending.then(() => undefined, () => undefined);
    return pending;
}
async function fundWallet(body, stored) {
    if (!ALLOW_PUBLIC_FUNDING)
        throw new Error("Public fork wallet funding is disabled");
    ...
    const quoteAmount = rawAmount(body, ["quoteAmount", "quoteUiAmount", "tokenAmount"], stored.quoteDecimals, DEFAULT_TOKEN_FUNDING_UI);
    ...
    return serializeWalletFunding(async () => {
        const { connection } = initializeRuntime();
        const existingAccount = await connection.getAccountInfo(owner, DEPLOYMENT_COMMITMENT);
        ...
        const basePlan = await prepareTokenFunding(owner, new PublicKey(stored.baseMint), baseAmount, new PublicKey(stored.baseTokenProgram));
        const quotePlan = await prepareTokenFunding(owner, new PublicKey(stored.quoteMint), quoteAmount, new PublicKey(stored.quoteTokenProgram));
        try {
            if (shouldMutateWalletLamports(sol))
            ...
            if (error instanceof ForkMutationOutcomeUncertainError)
                throw error;
...
async function txResponse(name, owner, stored, transaction, extra = {}) {
    return {
        success: true,
        data: {
            action: name,
...
async function routeWithoutDeploymentEnvelope(req, body, expectedDeployment) {
    const expectedDeploymentFingerprint = deploymentIdentityFingerprint(expectedDeployment);
    const url = new URL(req.url ?? "/", "http://localhost");
    const path = url.pathname.replace(/\/$/, "") || "/";
    if (req.method === "GET" && path === "/health") {
        initializeRuntime();
        const shouldVerifyPublicRpc = process.env.DUSK_REQUIRE_PUBLIC_RPC_URL === "true" ||
        ...
        const publicRpcVerification = shouldVerifyPublicRpc
            ...
            : null;
        const prebootstrappedMarkets = process.env.DUSK_REQUIRE_PREBOOTSTRAPPED_MARKETS === "true"
        ...
        return forkHealthPayload({
            ...
            publicRpcFilterVerified: publicRpcVerification?.filterVerified === true,
    ...
    const bootstrappedMarkets = await bootstrapMarkets(expectedDeploymentFingerprint);
    ...
    if (req.method === "GET" && path === "/api/v2/fork/bootstrap-evidence") {
        return {
            success: true,
            data: await bootstrapEvidencePayload(bootstrappedMarkets, expectedDeploymentFingerprint),
        };
    }
    ...
    if (req.method === "GET" && path === "/api/v2/markets") {
        const requestedLimit = Number(url.searchParams.get("limit") ?? bootstrappedMarkets.length);
    ...
    if (req.method === "GET" && userPositionsMatch) {
        ...
        return {
            success: true,
            data: {
                positions: (await Promise.all(positionMarkets.map((market) => userPositionsPayload(wallet, market, positionId)))).flat(),
            },
        };
    }
    ...
    if (req.method === "GET" && path === "/api/v2/fork/yield-account") {
        ...
        return {
            success: true,
            data: {
                yieldAccount: await yieldAccountPayload(stored, owner, lpMint, asset, tokenKind),
            },
        };
    }
    if (req.method !== "POST") {
    ...
    if (path === "/api/v2/fork/tx/create-market") {
        ...
        if (typeof body.baseMint !== "string" ||
            typeof body.quoteMint !== "string") {
            throw new Error("baseMint and quoteMint are required in canonical market order");
        }
    ...
    if (path === "/api/v2/fork/tx/update-protocol-revenue") {
        const revenueDistribution = body.revenueDistribution;
        const protocolAuctionSplit = body.protocolAuctionSplit;
        const transaction = await buildUpdateProtocolRevenueTx({
            ...
            maxReferralInterestShareBps: body.maxReferralInterestShareBps == null
                ? null
    ...
    if (path === "/api/v2/fork/tx/update-protocol-auction-config") {
        ...
        const auctionParamsBody = body.params;
    ...
    if (path === "/api/v2/fork/tx/settle-protocol-auction") {
        ...
        if (!acceptedMintInfo)
            throw new Error(`Accepted mint ${acceptedMint.toBase58()} does not exist`);
        const acceptedTokenProgram = acceptedMintInfo.owner;
        if (!acceptedTokenProgram.equals(TOKEN_PROGRAM_ID) &&
        ...
        const transaction = await buildSettleProtocolAuctionTx({
            ...
            maxPaymentAmount: parseUnits(String(body.maxPaymentAmount ?? "0"), acceptedMintAccount.decimals),
        });
    ...
    if (path === "/api/v2/fork/tx/create-parameter-proposal") {
        ...
        const decimals = await mintDecimals(new PublicKey(stored.ylpMint), TOKEN_2022_PROGRAM_ID);
        ...
        return txResponse("create-parameter-proposal", proposer, stored, built.transaction, {
    ...
    if (path === "/api/v2/fork/tx/support-parameter-proposal") {
        ...
        return txResponse("support-parameter-proposal", supporter, stored, built.transaction, {
    ...
    if (path === "/api/v2/fork/tx/withdraw-parameter-support") {
        ...
        return txResponse("withdraw-parameter-support", supporter, stored, built.transaction, {
            proposal: proposal.toBase58(),
            proposalSupport: built.proposalSupport.toBase58(),
            bootstrapSigned,
        });
    ...
    if (path === "/api/v2/fork/tx/transfer-lp") {
        ...
        const lpMint = tokenKind === "ylp"
            ...
            : asset === "base"
                ? m.baseHlpMint
                : m.quoteHlpMint;
        const mint = await getMint(initializeRuntime().connection, lpMint, "confirmed", TOKEN_2022_PROGRAM_ID);
    ...
    if (path === "/api/v2/fork/tx/preview-swap") {
        ...
        const transaction = await buildPreviewSwapTx({
            ...
            market: stored,
            assetIn,
            exactAssetIn: rawAmount(body, ["exactAssetIn", "amountIn", "amount"], assetIn === "base" ? stored.baseDecimals : stored.quoteDecimals, "1"),
        });
    ...
    if (path === "/api/v2/fork/tx/preview-borrow-capacity") {
        ...
        const transaction = await buildPreviewBorrowCapacityTx({
            ...
            market: stored,
            collateralAsset,
            collateralAmount: rawAmount(body, ["collateralAmount"], collateralAsset === "base" ? stored.baseDecimals : stored.quoteDecimals, "1"),
    ...
    if (path === "/api/v2/fork/tx/preview-borrow-position") {
        ...
        return txResponse("preview-borrow-position", owner, stored, await buildPreviewBorrowPositionTx({ owner, market: stored, positionId }), { borrowPositionId: positionId.toBase58() });
    }
    ...
    if (path === "/api/v2/fork/tx/swap") {
        ...
        const transaction = await buildSwapTx({
            ...
            minAssetOut: rawAmount(body, ["minAssetOut", "minAmountOut"], assetIn === "base" ? stored.quoteDecimals : stored.baseDecimals, "0"),
        });
    ...
    if (path === "/api/v2/fork/tx/deposit-collateral") {
        ...
        const positionId = optionalPublicKey(body.positionId ?? body.borrowPositionId) ??
        ...
        const transaction = await buildDepositCollateralTx({
            ...
            positionId,
            marketAsset,
            depositAmount: rawAmount(body, ["depositAmount", "amount"], marketAsset === "base" ? stored.baseDecimals : stored.quoteDecimals, "1"),
        });
    ...
    if (path === "/api/v2/fork/tx/withdraw-collateral") {
        ...
        const transaction = await buildWithdrawCollateralTx({
            ...
            positionId,
            marketAsset,
            withdrawAmount: rawAmount(body, ["withdrawAmount", "amount"], marketAsset === "base" ? stored.baseDecimals : stored.quoteDecimals, "1"),
            minAssetAmountOut: rawAmount(body, ["minAssetAmountOut", "minAmountOut"], marketAsset === "base" ? stored.baseDecimals : stored.quoteDecimals, "0"),
    ...
    if (path === "/api/v2/fork/tx/borrow") {
        ...
        const minDebtAmountOut = body.minDebtAmountOut != null && body.minDebtAmountOut !== ""
    ...
    if (path === "/api/v2/fork/tx/repay") {
        ...
        const transaction = await buildRepayTx({
            ...
            positionId,
            repayAsset,
            repayAmount: rawAmount(body, ["repayAmount", "amount"], repayAsset === "base" ? stored.baseDecimals : stored.quoteDecimals, "1"),
        });
    ...
    if (path === "/api/v2/fork/tx/remove-leverage-margin") {
        ...
        const transaction = await buildRemoveLeverageMarginTx({
            ...
            debtAsset,
            amount,
            minAmountOut: body.minAmountOut == null || body.minAmountOut === ""
                ? amount
    ...
    if (path === "/api/v2/fork/tx/deposit-single-sided") {
        ...
        const transaction = await buildDepositSingleSidedTx({
            ...
            market: stored,
            targetAsset,
            depositAmount: rawAmount(body, ["depositAmount", "amount"], targetAsset === "base" ? stored.baseDecimals : stored.quoteDecimals, "1"),
            minHlpAmount: rawAmount(body, ["minHlpAmount"], targetAsset === "base" ? stored.baseDecimals : stored.quoteDecimals, "0"),
        });
    ...
    if (path === "/api/v2/fork/tx/withdraw-single-sided") {
        ...
        const transaction = await buildWithdrawSingleSidedTx({
            ...
            market: stored,
            targetAsset,
            hlpAmount: rawAmount(body, ["hlpAmount", "amount"], targetAsset === "base" ? stored.baseDecimals : stored.quoteDecimals, "1"),
            minTargetAmountOut: rawAmount(body, ["minTargetAmountOut", "minAmountOut"], targetAsset === "base" ? stored.baseDecimals : stored.quoteDecimals, "0"),
        });
...
export async function route(req, body) {
    ...
    const value = await routeWithoutDeploymentEnvelope(req, body, before);
    let after;
    try {
        after = await deploymentEnvelope(maximumResponseSourceSlot(value));
    }
    catch (error) {
        if (error instanceof DeploymentIdentityChangedError)
            throw error;
        const detail = error instanceof Error ? error.message : String(error);
        throw new DeploymentIdentityChangedError(`Unable to re-observe Surfpool deployment identity after handling the request: ${detail}`);
    }
    if (beforeFingerprint !== deploymentIdentityFingerprint(after)) {
        throw new DeploymentIdentityChangedError("Surfpool deployment identity changed while handling the API request");
    }
    return { ...value, deployment: after };
}
